...
_SURFACE_FLAGS: tuple[tuple[str, str], ...] = (
    ...
    (
        "VOICE_INPUT_ENABLED",
        "Голосовые не распознаются — на голосовое человек получает «пока понимаю только текст».",
    ),
    (
        "VOICE_CROSS_BORDER_ALLOWED",
        "Голосовые не уходят в зарубежного провайдера распознавания (OpenAI) — при включённом "
        "VOICE_INPUT_ENABLED человек получает «сейчас не могу разобрать голосовое».",
    ),
    (
