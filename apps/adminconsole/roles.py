...
* ``tenancy.tenantstaff`` / ``tenancy.staffinvite`` — доступы к салону
  и их приглашения смотрятся поперёк салонов, но правятся только
  продуктовыми действиями (DRF-2061);
...
EDITOR_DENIED_MODELS: frozenset[str] = frozenset(
    {
        ...
        # Карточки доступов и приглашений (DRF-2061) — показ поперёк
        # салонов под правом ``tenancy.platform_operations``, только
        # чтение: доступ выдаётся и отзывается продуктовыми действиями
        # с провенансом, а в строке приглашения лежит его дайджест.
        # Экран запрещает правку сам; здесь — второй слой.
        "tenancy.staffinvite",
        "tenancy.tenant",
        "tenancy.tenantstaff",
    }
