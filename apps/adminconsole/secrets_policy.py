...
редактируются. Записи про ``catalog.catalogmaster`` здесь больше нет:
с DRF-1515 ``invite_token`` маскируется в самом каталоге, у владельца
поля (``CatalogMasterAdmin.exclude``), и внешняя заплатка снята.
...
SECRET_FIELDS: dict[str, tuple[str, ...]] = {
    ...
    # ``catalog.catalogmaster.invite_token`` здесь больше не прячем: с
    # DRF-1515 его маскирует сам каталог (``CatalogMasterAdmin.exclude``).
}
