...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        ...
        if canon:
            ...
            # Нижняя граница по самому надёжному состоянию зеркала — и предел
            # этой границы назван тут же, чтобы число не выглядело полным.
            w(
                "решает:  ЗЕРКАЛО (канон не спрошен) — это НИЖНЯЯ ГРАНИЦА: отмену "
                "зеркало знает (след состоявшегося действия), а про неоплату не "
                "узнает НИКОГДА — события про переход в awaiting_payment не "
                "существует ни на одной стороне (DRF-2519). Для полного числа: --canon"
            )
        w("")
