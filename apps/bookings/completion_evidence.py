...
#:
#: **Решение владельца 25.09.2026 (DRF-2519): только** ``completed``.
#:
#: Это видимое изменение, и оно НЕ регрессия. Раньше здесь стоял и
#: ``confirmed`` — с доводом «канон присылает ``booking.completed`` не всегда,
#: а требовать ``completed`` значило бы не закрывать почти ничего». Довод был
#: верен про доставку и неверен про предмет: замер стенда 25.09 показал, что у
#: зеркала **нет столбца** ``updated_at`` — строка пишется один раз, при
#: заведении, и канонические переходы в неё не приезжают. Значит «зеркало не
#: возражает» на деле означало «копия, снятая при заведении, не возражает», и
#: два неоплаченных визита уже помечены состоявшимися: канон ушёл в
#: ``awaiting_payment`` за 4 ч 18 мин и за 21 час ДО штампа.
#:
#: Цена названа владельцу до решения и принята им: автозакрытие визитов почти
#: перестаёт работать, и мастер увидит разницу. Выбрано остановить
#: производство ложных штампов.
#:
#: Тому, кто увидит упавшее число: **это не сбой и откатывать нечего.**
#: Настоящая починка — читать канон по ``RemoteBookingProxy.appointment_id``
...
#: Отдельный лист, а не тихая правка здесь.
MIRROR_ALLOWS: tuple[str, ...] = ("completed",)
...
#: неоплата со стенда.
#:
#: Набор оставлен как есть намеренно: с DRF-2519 отказ даёт уже само
#: отсутствие ``completed``, и этот перечень больше не единственная защита —
#: но он остаётся ЧИТАЕМЫМ ответом на вопрос «почему не штампуем», а значит и
#: причиной в счётчиках. Слить его в «всё, что не completed» значило бы
#: потерять машинные причины отказа.
