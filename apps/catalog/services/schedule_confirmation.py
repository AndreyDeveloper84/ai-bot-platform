...
def clear_confirmation(*, tenant_id: Any, ayla_user_id: Any, reason: str) -> int:
    ...
    Зовут её двое, и **работает сегодня только второй**:

    * событие ``master.schedule.updated`` (``apps.eventbus.consumers.schedule``)
      — по контракту §3.11 верное место, но замер 09.09.2026 показал, что
      топик не приходил ни разу за всю историю. Мгновенный сброс, которого
      сегодня не бывает;
    * подметание ``apps.catalog.tasks.sweep_schedule_confirmations`` —
      единственный механизм, который реально снимает подтверждение. Сброс
      с задержкой **до одного цикла обхода**, а не мгновенный.

    Разницу называем вслух: правило 4 звучит как «любое изменение часов
    отменяет подтверждение», а исполняется как «отменяет не позднее чем
    через цикл». Выдать второе за первое значило бы обещать свойство,
    которого у механизма нет.
