...
def test_tenant_address_absent_today_leaves_column_untouched(tenant):
    """Ключа ``tenant_address`` в строке нет → колонка ``None``.

    Это честное пустое, а не дефект: у салона адрес не заполнен (каталог шлёт
    ``null``) или строка от каталога старше DRF-1587 (DRF-1954). Здесь важно
