...
def sweep_schedule_confirmations() -> dict[str, int]:
    ...
    **на 09.09.2026 другого работающего пути инвалидации нет вовсе.**
    ...
    это утверждение станет неверным, не изменившись ни буквой. Читателю
    после этой даты: проверь ``EVENT_INGEST_ALLOWED_EVENTS`` в контейнере
    и ``IngestDedupe`` по имени топика, прежде чем опираться. Если
    событийный путь открыт, обход остаётся страховкой, а не единственным
    механизмом.
