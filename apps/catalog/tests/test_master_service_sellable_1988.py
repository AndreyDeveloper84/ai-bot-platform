...
def _service(
    tenant: Tenant, name: str = "Массаж шейно-воротниковой зоны", slug: str = "neck"
) -> CatalogService:
...
def test_parser_reads_unsellable_edge_with_reason(tenant):
    ...
    dto = _parse_specialist_service(
        _row(s, m, sellable=False, unsellable_reason="price_below_minimum")
    )
...
def test_parser_marks_sellable_key_absent(tenant):
    ...
    assert getattr(dto, "sellable_key_present", None) is False, (
        "разбор не различает отсутствие ключа sellable"
    )
...
def test_unsellable_without_reason_is_refused_by_the_database(tenant):
    ...
    with pytest.raises(IntegrityError), transaction.atomic():
        MasterService.all_tenants.create(
            tenant=tenant, master=m, service=s, sellable=False, unsellable_reason=""
        )
...
def test_create_stores_unsellable_and_reason(tenant):
    ...
    upsert_master_services(
        tenant,
        [_dto(m, s, edge_id=str(uuid.uuid4()), sellable=False, reason="price_below_minimum")],
    )
    _require_column()
...
def test_update_to_unsellable_writes_only_changed_fields(tenant):
    ...
    upsert_master_services(
        tenant, [_dto(m, s, edge_id=edge, sellable=False, reason="price_below_minimum")]
    )
...
def test_sellable_queryset_excludes_unsellable_edges(tenant):
    _require_column()
    assert hasattr(MasterService.all_tenants.all(), "sellable"), (
        "нет MasterServiceQuerySet.sellable()"
    )
    m = _master(tenant)
    ...
    ids = set(
        MasterService.all_tenants.filter(tenant=tenant)
        .sellable()
