...
EXPECTED: dict[str, tuple[int, str, str]] = {
    ...
    "apps/master_api/services/assistant_actions.py": (
        1,
        SELLABLE,
        "DRF-2153 (М-5): ассистент мастера резолвит его услугу для предложения "
        "записи — только продаваемое ребро (предикат sellable()); создаёт запись "
        "тот же сервис, что форма и стойка, после подтверждения человеком",
    ),
    # ── не путь продажи ──
