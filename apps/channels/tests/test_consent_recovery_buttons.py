...
Кнопка живёт только на глобальном пути (``current_tenant() is None``): на
салонном ``WelcomeSkill._render_consent_granted`` ставит ``consent_at``, но НЕ
пишет ConsentRecord, а отказы читают журнал — кнопка там вернула бы человека к
тому же отказу. Решение главного окна 16.09: салонный путь — отдельный лист.
