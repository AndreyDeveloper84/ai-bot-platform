...
def revoke_data_storage(bot_user: "BotUser") -> "DeleteCascadeResult":
    ...
    with transaction.atomic():
        ...
        #
        # Согласие дневника и сканера (DRF-1948: стоит поверх PERSONAL_DATA и
        # отзывается вместе с ним) снимает каскад выше — это строка реестра
        # ``food_diary_processing`` в ``_PERSONAL_DATA_CASCADE`` (DRF-1963, D6),
        # а не ``update`` колонки. Повторная выдача PERSONAL_DATA его не
        # возвращает — согласие дневника даётся заново на своём экране.
