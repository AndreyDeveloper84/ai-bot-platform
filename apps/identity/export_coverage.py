...
NON_REGISTRY_STORES: Mapping[str, str] = {
    ...
    "conversations.AiDraft.content": (
        ...
        "клиента. Текст очищается, когда черновик отправлен или отклонён, и "
        "при «удалить всё» — вместе с обезличиванием переписки (DRF-1369)."
    ),
    "redis.short_term": (
        ...
        "«удалить всё» очищается сразу, не дожидаясь срока."
    ),
    "redis.pii_tokenmap": (
        "Техническая обратная карта «токен → настоящее значение» для "
        "маскировки телефона в запросах к языковой модели: модель видит "
        "токен, а не номер. В Postgres не пишется, живёт в Redis на срок "
        "диалога плюс час. Сам телефон выгружается в разделе channel_shell "
        "(identity.BotUser.phone) — здесь лишь его служебная копия; при "
        "«удалить всё» карта очищается сразу."
    ),
}
