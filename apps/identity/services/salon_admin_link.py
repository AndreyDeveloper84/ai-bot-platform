...
(``apps/identity/services/staff_invites.py``) при ``role=admin``. Ядро одно
на три двери — «Выдать роль в салоне», «Сменить роль» и ввод кода,
выданного оператором, — и потому каталожная половина не может разойтись
с ботовой ни на одной из них.
...
* **Отказ — по имени, строки нет.** :class:`CatalogAdminLinkRefused`
  наследует ``InviteError``: ядро вызывается внутри транзакции
  вызывающего, исключение откатывает всё (в том числе ``used_at`` кода —
  код остаётся действующим, человек повторит после починки). Текст «что
