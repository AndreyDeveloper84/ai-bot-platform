...
* a8 — владельца передавали → тенант всё равно доказан, в том числе когда
  слаг дала личность СНЯТОГО владельца, а действующий — уже другой человек:
  `deactivated_at` — мягкая деактивация, вопрос команды — происхождение, а
  не нынешние полномочия;
* a9 — диагностика смотрит обе двери: поломка поиска владельцев уронила бы
  доказанные тенанты в «без владельца», и строка про пересчёт читалась бы
  здоровой.

Числа читаются ключом отчёта, а не вхождением подстроки: «: 1» совпало бы и
с «: 11». Там, где в счёт попадают служебные тенанты окружения (миграция
`0014_seed_global_bot_tenant` заводит тенанта без владельца), сверяется
ПРИРОСТ — абсолютное число мерило бы окружение, а не команду.
"""
...
class TestA3PrefixIsNotProof:
    def test_a_solo_looking_slug_with_another_identity_is_refused(self) -> None:
        ...
        assert _count(err, "из них слаги соло-вида, пересчёт отказал (ожидается 0)") == 1
...
class TestA8OwnerHandoverStillProves:
    ...
    def test_the_origin_identity_may_be_the_revoked_one(self) -> None:
        """Настоящая форма передачи: тенант завёл один человек (его личность и
        дала слаг), а действующий владелец — уже другой. Сузь правило до
        «действующих владельцев» — и доказанный тенант уехал бы в «пересчёт не
        сошёлся», причём молча."""
        origin = _identity()
        slug = _solo_tenant_slug(CHANNEL, origin)
        tenant = Tenant.all_objects.create(slug=slug, name="Мастерская")
        _owner(tenant, origin, revoked=True)
        _owner(tenant, _identity())
...
class TestA9TheDiagnosticWatchesBothDoors:
    def test_a_solo_shaped_tenant_without_owners_is_counted_too(self) -> None:
        """Поломка поиска владельцев — форма исходного блокера — уронила бы
        доказанные тенанты в «без владельца», и строка про пересчёт читалась
        бы здоровой."""
        proven, _ = _solo_tenant()
        _out, before = _run()

        Tenant.all_objects.create(slug=_solo_tenant_slug(CHANNEL, _identity()), name="Ничей")
        out, err = _run()

        assert proven.slug in _slugs(out)  # наличие: доказанный печатается
        key = "из них слаги соло-вида, владельца не нашли (ожидается 0)"
        assert _count(err, key) == _count(before, key) + 1
