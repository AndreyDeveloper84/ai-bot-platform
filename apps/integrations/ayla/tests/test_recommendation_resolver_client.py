...
@pytest.mark.parametrize(
    "candidate_ref",
    [
        pytest.param({"id": "11111111-1111-1111-1111-111111111111"}, id="kind отсутствует"),
        pytest.param(
            {"kind": "MASTER", "id": "11111111-1111-1111-1111-111111111111"},
            id="kind вне закрытого множества",
        ),
        pytest.param(
            {"kind": 1, "id": "11111111-1111-1111-1111-111111111111"},
            id="kind не строка",
        ),
    ],
)
def test_candidate_kind_is_checked_alongside_the_id(candidate_ref):
    """`kind` проверяется наравне с `id` — иначе предмет рекомендации неизвестен.

    Расхождение нашла клиентская сторона (T7) и принесла, а не подогнала
    молча: контракт §4.3 объявляет `candidate_ref {kind, id}`, а здесь
    проверялся один `id`. Существовал ответ, который бот принимал,
    а мини-приложение отвергало, — то есть две половины одной границы
    расходились в том, что считают валидным.

    Довод сильнее формального соответствия: без `kind` «услуга»
    и «мастер» неразличимы, а полка услуг, молча принявшая `PROVIDER`, —
    это подстановка другого предмета, запрещённая §14.4.

    Неизвестное значение — тоже нарушение: множество закрыто (§5, K1),
    и `MASTER` вместо `PROVIDER` означает, что источник говорит на
    диалекте, которого потребитель не знает. Разбирать неизвестное
    запрещено (§9.4) — по той же причине, по которой запрещено разбирать
    неизвестную мажорную версию.
    """
    violation = decision_contract_violation(_body(_candidate(candidate=candidate_ref)))
    assert violation is not None
    assert "candidate.kind" in violation


def test_all_four_declared_kinds_pass():
    """Положительная стража: проверка не превратилась в «только PROVIDER».

    Без неё три теста выше зеленели бы и на коде, который отвергает
    вообще всё, кроме сегодняшнего единственного вида кандидата, —
    и `SERVICE` сломался бы в тот день, когда рекомендовать начнут
    услуги, а не мастеров.
    """
    for kind in ("SERVICE", "OFFER", "PROVIDER", "SLOT"):
        body = _body(
            _candidate(
                candidate={
                    "kind": kind,
        ...
        assert decision_contract_violation(body) is None, kind
