...
class TestTenantDay:
    """DRF-1111/DRF-1161 — the reconciliation sweep's canonical read.

    ``tenants/me/day/`` is the only tenant-wide appointment read Ayla
    exposes to the service; the detector fans it out over a bounded date
    window, so the wire shape is pinned here.
    """

    def test_gets_the_day_endpoint_with_the_date(self) -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            seen["url"] = str(request.url)
            seen["headers"] = dict(request.headers)
            return httpx.Response(200, json={"data": {"date": "2026-08-25", "masters": []}})

        day = _client(handler).get_tenant_day(
            actor_external_id=ACTOR,
            tenant_slug="formula-tela",
        ...
        assert day == {"date": "2026-08-25", "masters": []}

    def test_refuses_a_missing_tenant_slug_before_the_wire(self) -> None:
        import datetime as dt

        client = _client(_ok)
        with pytest.raises(SalonValidationError, match="tenant_slug"):
            client.get_tenant_day(
                actor_external_id=ACTOR,
                tenant_slug="",
                date=dt.date(2026, 8, 25),
            )

    def test_forbidden_means_the_actor_is_no_admin(self) -> None:
        import datetime as dt

        def handler(request: httpx.Request) -> httpx.Response:
            return httpx.Response(403, json={"error": {"message": "not admin"}})

        with pytest.raises(SalonForbidden):
            _client(handler).get_tenant_day(
                actor_external_id=ACTOR,
                tenant_slug="formula-tela",
                date=dt.date(2026, 8, 25),
            )

    def test_unrecognised_payload_is_unavailable_not_empty(self) -> None:
        """A success shape we do not recognise must never read as
        «нет записей» — that would silence the detector exactly when the
