...
def _handle_create(tenant: Tenant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    # DRF-1558 — НЕ переводить на ``channel_user_id``, хотя напоминания теперь
    # уходят именно по нему. Здесь ``chat_id`` перестал быть адресом и остался
    # признаком: «этот человек хоть раз говорил с нашим ботом». Строка, попавшая
    # сюда, найдена **сопоставлением телефона** из YClients — согласия у нас на
    # неё может не быть вовсе, а ``send_due_reminders`` ничем не гейтится
    # (`apps/notifications/proactive.py` перечисляет проактивные швы, и этого
    # среди них нет). Замена признака на идентификатор человека, который есть
    # всегда, открыла бы напоминания тем, кто бота не открывал: MAX по
    # ``user_id`` заводит диалог сам, и мы написали бы первыми незнакомому
    # человеку. Пропущенное напоминание — цена меньшая.
