...
## Линтер: одно правило, ошибкой (DRF-2388)

`npm run lint` — `eslint` с единственным правилом `react-hooks/rules-of-hooks`,
включённым **ошибкой**, и обязательным шагом в CI. Конфигурация и причины —
в `eslint.config.mjs`; там же именованный список файлов с долгом, который
**может только сокращаться**.

До 24.09 конфигурации ESLint здесь не было вовсе. Замер при включении: **8
нарушений правила хуков в 3 файлах** из 411 — и **26 мёртвых директив
`eslint-disable`**, написанных для линтера, которого не существовало. Второе
