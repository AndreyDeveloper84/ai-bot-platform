 ...
 *
 * # Особая категория (DRF-1453)
 *
 * Раскрытие обязано называть медданные отдельно, иначе оно описывает не тот
 * режим, в котором они на самом деле обрабатываются. Строка ниже утверждает
 * ровно то, что подтверждается кодом: без отдельного разрешения нутриционная
 * поверхность отказывает (сторож в
 * apps/orchestrator/nutrition_context.py), а само разрешение выдаётся и
 * отзывается строкой «Питание и здоровье» в R2. Ничего не обещаем сверх —
 * правило §14 про необеспеченные обещания действует и здесь.
 */
...
const HEALTH_DATA_PARAGRAPH =
  "Питание и здоровье закон относит к особой категории. Эти данные " +
  "обрабатываются только по отдельному разрешению — его можно дать и " +
  "отозвать выше, в строке «Питание и здоровье». Без него Ayla не " +
  "учитывает питание в разговоре.";
