 ...
 *
 * Отдельный компонент, а не режим `AdminTabBar`. Мостовая панель
 * обслуживает пять разделов и заперта тестом
 * (`App.receptionSurface.test.tsx` требует ровно пять подписей у
 * владельца и три у ресепшн); подмешать в неё четвёртый набор значило
 * бы либо сломать этот тест, либо спрятать пилот за флагом внутри чужой
 * панели. Пилот и мост живут параллельно, и панели у них тоже две.
 *
