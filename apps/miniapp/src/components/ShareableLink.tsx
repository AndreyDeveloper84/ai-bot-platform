...
export function ShareableLink({
  ...
  return (
    <div className={className ? `shareable ${className}` : "shareable"}>
      ...
      {/*
        Итог копирования объявляется вслух.

        Смена подписи кнопки на «Скопировано» видна глазами и невидима
        скринридеру: фокус остаётся на кнопке, а её имя меняется без
        события, о котором стоит сообщать. Отдельная живая область —
        единственное, что делает результат слышимым. Отказ идёт
        `role="alert"` (перебивает), успех — `role="status"` (дожидается
        паузы): «скопировалось» не стоит того, чтобы обрывать читаемое.
      */}
      {state === "copied" && (
