...
describe("C04.5 — safety boundary (DRF-1767)", () => {
  ...
  it("сторож диагнозов стережёт класс, а не одно написание", () => {
    // Падежи, уменьшительные и вопросные формы (окно безопасности, 12.09):
    // диагноз ставится и словом, и вопросом «опасно ли это».
    for (const phrase of [
      "похоже на заболевание кожи",
      "поставить диагноз",
      "без диагноза",
      "с таким диагнозом",
      ...
      expect(DIAGNOSIS_WORDS.test(phrase), phrase).toBe(true);
    }
    expect(DIAGNOSIS_WORDS.test("Здесь я не помощник")).toBe(false);
    expect(DIAGNOSIS_WORDS.test("с красотой и записью помогу с радостью")).toBe(false);
  });
});
