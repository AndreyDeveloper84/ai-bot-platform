...
describe("Д31 — сняты два блока из четырёх", () => {
  ...
  //
  // Стало: полки нет И запроса нет.
  //
  // Что при этом ПЕРЕСТАЛО проверяться — говорю прямо, потому что сперва
  // написал, будто случай «переехал», а он не переехал. Состояние
  // «источник ответил полно, а полка всё равно тёмная» теперь НЕДОСТИЖИМО
  // по построению: данные в срез кладёт только зажжённая полка. Значит
  // само условие `shelfOn` в вёрстке узлом не проверяется ничем — это
  // цена решения, а не недосмотр. В `…shelfRequest2348.test.tsx` лежит
  // другая пара: тёмная не ходит в сеть, зажжённая ходит и показывает
  // привезённое.
