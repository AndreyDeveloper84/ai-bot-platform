...
describe("Запрос за полкой ходит только при зажжённой полке (DRF-2348)", () => {
  it("полка обездвижена — за её данными не ходим вовсе", async () => {
    ...
    // всем остальным, иначе «обращения нет» окажется правдой просто
    // потому, что ничего ещё не начиналось.
    await waitFor(() =>
      expect(screen.getByText("План на сегодня")).toBeInTheDocument(),
    );
