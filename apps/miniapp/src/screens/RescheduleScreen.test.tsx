 ...
 * старую карточку `/my-visits/:id`, которая не рисовала ни статус
 * оплаты, ни сумму, ни выставленную оценку и не имела кнопки «Оценить
 * визит». То есть человек после переноса записи терял возможность
 * оценить визит.
 *
 ...
 * (страховка для внешних ссылок, ушедших наружу ранее).
 *
 * DRF-1625: сама старая карточка (`MyVisitDetailScreen`) снята вместе
 * со своим адресом, поэтому сравнивать больше не с чем — но проба
 * «СТАРАЯ КАРТОЧКА» ниже остаётся смонтированной в ЭТОМ роутере:
 * маршрут назначения проверяется по тому, ЧТО смонтировалось, и без
 * пробы на старом адресе утверждение «приземлились в новую» было бы
 * отсутствием без контроля.
 */
...
describe("почему это не косметика: что даёт каноническая карточка", () => {
  // Оценить визит можно только с той карточки, где есть кнопка. Пока
  // перенос приземлял на старую (снята в DRF-1625), человек её не видел.
