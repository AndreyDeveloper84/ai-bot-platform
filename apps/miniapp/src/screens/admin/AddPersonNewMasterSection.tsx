 ...
 *
 * Экспортируется ради теста: правил здесь шесть, а глазами на экране
 * проверяется одно за прогон.
 *
 * # Четыре вещи, которые эта функция обязана держать врозь
 *
 ...
 *    сторону.
 * 4. **«Ссылка ниже» говорится, только когда ссылка ниже есть.**
 *
 * # Про четвёртое — отдельно, потому что первая версия его провалила
 *
 * Первая редакция читала `invite_link` ровно в одной ветке из шести, а
 * остальные пять безусловно советовали «отправьте ссылку ниже». При
 * пустом `invite_link` экран рисует под этим текстом красное «Ссылки
 * нет: в этом контуре не настроен салонный бот» — то есть обещание и
 * его опровержение вставали рядом, на одном экране.
 *
 * Это ДОСЛОВНО тот дефект, который эта задача чинит одним уровнем выше:
 * прежний экран говорил «перешлите ссылку ниже», когда ссылки там не
 * было. Он воспроизвёлся здесь, в функции, написанной специально чтобы
 * его убрать, — и это стоит помнить: «мы про это знаем» не мешает
 * повторить.
 *
 * Поэтому совет о ссылке живёт в одном месте — `handover()` — и
 * спрашивает про неё каждый раз.
 */
export function deliveryNotice(
  ...
  /**
   * Что делать дальше — совет, который зависит от наличия ссылки.
   *
   * Без ссылки советовать «отправьте ссылку» значит послать читателя
   * искать на экране то, чего там нет.
   *
   * Замена по умолчанию нарочно жёсткая: если сообщение НЕ ушло и
   * ссылки нет, приглашение не существует ни в каком виде, и владельцу
   * надо сказать это прямо, а не смягчить. Ветки, где сообщение всё же
   * ушло, передают своё `whenNoLink` — там приглашение живо, просто
   * второго способа его повторить у владельца нет.
   */
  const handover = (
    whenLink: string,
    whenNoLink = "Передать приглашение сейчас нечем: ссылки в этом контуре " +
      "нет (подробнее ниже). Сообщите нам — сами вы это не почините.",
  ): string => (hasLink ? whenLink : whenNoLink);

  //: Сообщение ушло, значит приглашение у человека уже есть — ссылка
  //: была бы вторым способом, а не единственным. Поэтому здесь мягкая
  //: замена, а не общая.
  const linkWouldHaveHelped =
    "если он(а) ничего не увидит, напишите нам: переслать приглашение " +
    "отдельно в этом контуре пока нечем.";

  if (result.max_dm_delivery === "delivered") {
    // Сегодня бэкенд этого не возвращает — доставку MAX не подтверждает,
    // и весь смысл соседней ветки в том, чтобы этого не выдумывать.
    // Ветка стоит здесь, потому что значение есть в типе: без неё
    // `delivered` проваливался бы в «не удалось» ниже и объявлял бы
    // поломкой самый удачный из возможных исходов.
    return {
      tone: "ok",
      text: `MAX подтвердил доставку сообщения ${firstName}.`,
    };
  }
  ...
  if (result.max_dm_delivery === "queued") {
    return {
      ...
      text:
        ...
        "у нас нет — " +
        handover(
          "если он(а) ничего не увидит, отправьте ссылку ниже сами.",
          linkWouldHaveHelped,
        ),
    };
  }
  ...
  if (result.max_dm_delivery === "skipped") {
    if (result.max_dm_error === "max_phone_lookup_deferred") {
      return {
        ...
        text:
          ...
          "ещё не построили. " +
          handover(
            "Отправьте ссылку ниже любым способом: она работает и без " +
              "переписки с ботом.",
          ),
      };
    }
    return {
      tone: hasLink ? "warn" : "danger",
      text: "Сообщение не отправлялось. " + handover("Передайте ссылку ниже сами."),
    };
  }
  ...
  if (result.max_dm_error === "no_entry_configured") {
    return {
      ...
      text:
        ...
        handover("Передайте ссылку ниже."),
    };
  }
  if (result.max_dm_error.startsWith("max_status_")) {
    ...
    return {
      ...
      text:
        ...
        "что такого MAX-аккаунта нет — проверьте написание. " +
        handover("Ссылка ниже работает независимо от аккаунта."),
    };
  }
  return {
    tone: hasLink ? "warn" : "danger",
    text:
      `Сообщение для ${firstName} отправить не удалось. ` +
      handover("Передайте ссылку ниже — она работает независимо от переписки."),
  };
}
