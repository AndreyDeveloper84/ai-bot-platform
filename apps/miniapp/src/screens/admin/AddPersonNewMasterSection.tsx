 ...
 * работающий способ передачи до владельца не доходил.
 *
 * **Личного сообщения больше нет.** Прежде экран честно называл
 * причину, по которой оно не ушло, — «MAX не нашёл такой аккаунт»,
 * «вход в мини-приложение не настроен». Решение владельца §44.4 от
 * 07.09.2026 убрало саму попытку: сообщение уходило КЛИЕНТСКИМ ботом и
 * достигало только тот чат, который уже существует, то есть незнакомому
 * мастеру не доходило никогда. Строка о результате говорила «не дошло»
 * почти на каждом приглашении. Вместе с попыткой ушли и она, и
 * `deliveryNotice`, и поля `max_dm_delivery` / `max_dm_error` в ответе.
 *
