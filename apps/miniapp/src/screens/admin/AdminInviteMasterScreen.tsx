            ...
            <>
              <p style={{ marginTop: "var(--s-3)" }}>
                {result.max_dm_delivery === "failed"
                  ? `Отправить сообщение ${inviteFirstName} в MAX не удалось — перешлите ссылку ниже в MAX-переписке.`
              ...
              {/*
                ...
                ⚠️ Формулировка «открывать внутри MAX» — не вежливость, а
                единственный рабочий путь. Вход в Mini App строится на
                `initData` от MAX; в браузере его нет, и `HelloScreen` отвечает
                «MAX не передал данные для входа». Бэкенд требует того же:
                `validate_invite_token` фильтрует токен ПО ТЕНАНТУ BotUser'а из
                сессии (apps/master_api/views.py:269) — без сессии тенант
                неизвестен и токен не проверить.

                Проверено вживую 30.08: владелец открыл ссылку в браузере и
                упёрся в этот экран. Текст самого DM при этом обещает
                «Не открывается в MAX? Используйте веб-версию» — обещание
                невыполнимо, заведено отдельно.
              */}
              {result.fallback_link && (
                ...
                >
                  {"Ссылка приглашения. Открывать её нужно ВНУТРИ MAX — "}
                  {"в браузере вход не сработает, приложению неоткуда узнать, "}
                  {"кто открыл. Отправляйте её в MAX-переписку: "}
