...
describe("на экране дня салона не осталось кнопок без правила", () => {
  ...
  it("контроль: сам измеритель умеет отвечать «нет»", () => {
    ...
    // Отрицательная половина берёт ЗАВЕДОМО НЕСУЩЕСТВУЮЩЕЕ имя, а не
    // `sheet__item` или `btn-link`. Те сегодня правила не имеют, но это
    // ДОЛГ, а не свойство: соседняя работа их одевает, и контроль,
    // опирающийся на чужой незакрытый долг, покраснеет от чужой починки.
    expect(hasRule("ayla-btn")).toBe(true);
    expect(hasRule("net-takogo-klassa-2448")).toBe(false);
  });
