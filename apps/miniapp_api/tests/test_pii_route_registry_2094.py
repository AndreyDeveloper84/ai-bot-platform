...
CUSTOMER_ROUTES: dict[str, Entry] = {
    ...
    "customer_recommendation": own(
        "data.what / data.subline (формулировка направления — словарь каталога, не человек)",
        "data.why (ПЕРЕСКАЗ И ЦИТАТЫ СЛОВ САМОГО ЧЕЛОВЕКА — то, что он сказал в этом пути)",
        "data.alternatives (другие направления того же словаря)",
        via=V + "customer_recommendation",
        note=(
            "карточка C04, которую этот человек уже увидел в чате: запись ищется по "
            "bot_user звонящего, чужая и стёртая отвечают тем же 404, что и "
            "несуществующая. Услуги, мастера, цены и слота здесь нет по построению — "
            "карточка их не содержит (B2/B3), поэтому третьих лиц в ответе нет вовсе"
        ),
    ),
