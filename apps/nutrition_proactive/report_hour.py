...
* Тихие часы (:data:`prefs.QUIET_START_HOUR` / :data:`prefs.QUIET_END_HOUR`)
  не меняются. Лист принимает час с 6 до 23; час 22/23 и 6..8 попадает
  в тихое окно, и планировщик, как и раньше, молчит
...
#: Допустимые часы отчёта (лист: «выбери час с 6 до 23»), включительно.
MIN_HOUR = 6
MAX_HOUR = 23

#: Тексты -- из листа DRF-2141. Слот ``{time}`` -- «HH:MM».
SET_CONFIRMATION = "Хорошо, итоги дня — в {time}."
NIGHT_REFUSAL = "Ночью не пишу — выбери час с 6 до 23."
ASK_REPLY = (
...
ASK_REPLY_OFF = "Итоги дня сейчас не присылаю. Скажи „присылай итоги в 20:00“, если хочешь получать."
...
_OFF_RE = re.compile(
    r"^(?:больше\s+)?не\s+присылай(?:те)?\s+(?:мне\s+)?(?:больше\s+)?" + _REPORT + r"(?:\s+больше)?$"
)
_ASK_RE = re.compile(
