...
def vet_outbound(text: str) -> tuple[str, str | None]:
    """Run a composed message past the outbound safety check.

    Returns ``(text, None)`` when it may go out, and ``("", reason)`` when
    it may not.

    Two of the three shapes ``evaluate_outbound`` looks for are live risks
    here even though this module writes none of them itself: the report
    passes ``summary.ai_comment`` through verbatim, and that sentence is
    generated by a model on Ayla's side that nothing on ours has reviewed.
    A proactive message is also the worst place for such a sentence to
    land, because the person did not ask a question and has no reason to
    read the answer sceptically.

    Where this deliberately differs from the pipeline: a blocked reply
    there is REPLACED with ``REPLACEMENT_TEXT`` («тут нужен человек…»),
    which is right for an answer somebody is waiting for. Unsolicited,
    that text is a non-sequitur — the person asked nothing, so there is
    nothing for an administrator to pick up. Here a hit means **send
    nothing at all** and log it. Silence is always an acceptable outcome
    for a message nobody requested.
    """
    ...
    *,
    ...
    for bot_user in selection.base_queryset():
        ...
        text, blocked_by = vet_outbound(
            render.render_water_reminder(water, proportional_ml=proportional)
        )
        if blocked_by:
            decisions.append(decide(blocked_by, detail=detail))
            continue
