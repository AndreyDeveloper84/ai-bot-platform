...
* d2 — «До ориентира по калориям осталось N ккал.» — при цели и когда
  съедено меньше ориентира; арифметика, не оценка (§85 §8);
* d3 — при ``total ≥ goal`` реплика «осталось» не появляется; «уложился в
  ориентир» снят (DRF-2215, T-2: оценка дня не говорится вообще);
...
class TestTheCaloriesTargetInTheReport:
    ...
    def test_d3_within_the_band_is_arithmetic_not_a_verdict(self) -> None:
        # DRF-2215 (T-2): «уложился в ориентир» снят — оценка дня; внутри
        # полосы остаётся арифметика «осталось».
        remark = render.goal_remark(
