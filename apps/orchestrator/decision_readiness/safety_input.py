...
class SafetyResult:
    ...
    #: §3: safety-контекст между ходами несёт `evidence_ref` — ссылку на
    #: свидетельство, по которой аудитор С КАТАЛОГОМ ПРАВИЛ найдёт, что именно
    #: сработало, а разговор при этом свидетельство не несёт (регулярке в
    #: состоянии разговора делать нечего — A-1). Для правил `pre_check` это
    #: `sha256(policy_version + сработавший шаблон)[:12]`: одна и та же
    #: политика и тот же шаблон дают ту же ссылку, смена любого из них — новую.
    #: `rule_id` называет КОРЗИНУ (причину), `evidence_ref` — конкретное
    #: свидетельство; входит в `digest_fields()`: это содержание вердикта.
    ...
    def digest_fields(self) -> tuple[str, ...]:
        ...
        return (
            ...
            # Свидетельство — содержание: тот же вердикт от другого шаблона
            # или под другой политикой — другое решение (§3, evidence_ref).
