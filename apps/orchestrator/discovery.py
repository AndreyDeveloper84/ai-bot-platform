...
# ---------------------------------------------------------------------------
# DRF-990, третий заход — чем тап discovery был В ИСТОРИИ
# ---------------------------------------------------------------------------
#
# Маршрутизация — про ТЕКУЩИЙ ход: ``_discovery_handoff_reply`` разбирает
# именно ``cb:discover:book:{T}:{M}[:{S}[:{ref}]]``, и payload обязан доехать
# до неё нетронутым. Историю же диалога консьерж читает на БУДУЩИХ ходах, и
# там строка «cb:discover:book:…» с ролью ``user`` выглядит как то, что
# человек написал ему словами.
#
# Боевой замер пилота 30.08: 68 сырых строк нажатия среди 355 реплик
# ``role=user``, и 55 из них — это семейство. Почти каждая пятая «реплика
# человека», которую читает консьерж, была служебной строкой.
#
# #1329 закрыл ГЛАГОЛ ``book:``. Прогон пятнадцати форм через
# ``handle_global_max_event`` показал, что закрыл он его целиком — все
# арности, включая деградации (битый id, перебор сегментов, пустой хвост), —
# а мимо гейта идёт ровно то, что глаголом ``book:`` не является:
# ``cb:discover:``, ``cb:discover`` и любой будущий глагол семейства.
# Гейт сторожил ГЛАГОЛ, а не СЕМЕЙСТВО.
#
# Здесь стоит семейство. Выбрано МОЛЧАНИЕ, и довод один на всё семейство:
#
#   * каждый payload несёт id мастера, салона или ссылку на сохранённый
#     запрос — то, чего человек не говорил и сказать не мог. Ровно довод
#     ``cb:catalog:*`` (DRF-1304), где гейт с самого начала стоит по
#     семейству;
#   * асимметрии, из-за которой анкете (DRF-990) и приветствию (#1329) дали
#     ФРАЗУ, здесь нет. Там часть шагов человек набирает руками, и молчание
#     оставило бы запись, где «30» есть, а пола нет. У воронки записи
#     набранных шагов нет вовсе: ``cb:discover:book:`` — её ПЕРВЫЙ шаг, все
#     последующие (``cb:book:*``, DRF-988) уже молчат, так что молчание
#     здесь однородно;
#   * то, чем человек ОТКРЫЛ воронку («хочу маникюр в пензе»), он набирает,
#     и эта реплика в истории остаётся всегда; ответы бота (карточки,
#     передача в салон) пишутся как обычно. Контекст следующего хода цел.
#
...
# ``apps/channels/tests/test_first_contact_c01.py``). Пропуск по
# ``startswith`` стёр бы такую реплику целиком — дефект хуже того, который
# здесь чинится: сырой payload модель хотя бы толкует, стёртую реплику она
# не увидит вовсе.

#: Строгая форма payload'а семейства: ``cb:discover`` и далее плоские
#: сегменты через двоеточие из алфавита id и base64url — без пробелов, без
#: кириллицы, в нижнем регистре у самого префикса. Покрывает всё, что
#: выкладывает :data:`CALLBACK_DISCOVER_BOOK_PREFIX` во всех арностях
#: (2–4 сегмента, DRF-962 / DRF-1324), пустой сегмент услуги
#: («…:{master}::{ref}»), деградации и любой будущий глагол семейства.
_DISCOVER_CALLBACK_RE = re.compile(r"^cb:discover(?::[A-Za-z0-9_=-]*)*$")


@dataclass(frozen=True)
class DiscoverTap:
    """Разбор тапа discovery глазами ИСТОРИИ диалога.

    Поле одно и всегда ``None``: у этого семейства нет формы, которая была
    бы репликой человека. Тип сохранён (а не заменён на ``bool``) ради
    единообразия с ``WelcomeTap`` / ``AnketaTap`` — вопрос у всех трёх
    резолверов один, «чем этот тап был как реплика», и разные ответы на него
    должны выглядеть одинаково устроенными. Появится в семействе кнопка,
    которая ЕСТЬ высказывание, — сюда ляжет её фраза, а не заведётся второй
    механизм рядом.
    """
...
def resolve_discover_tap(text: str) -> DiscoverTap | None:
    """Разобрать тап discovery; ``None`` — «это не тап discovery».
    ...
    собственную реплику.
    """
    if not _DISCOVER_CALLBACK_RE.match((text or "").strip()):
        return None
    return DiscoverTap()
