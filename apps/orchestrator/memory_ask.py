...
_DIET_WORDS = (
    ...
    ("halal", r"халял"),
    ("kosher", r"кошер"),
    # DRF-2392: ``omnivore`` («без ограничений») здесь ОТСУТСТВОВАЛ, и «ем
    # всё» ложилось как ``other`` — а ``other`` в анкете означает «и я напишу
    ...
    # Со границей слова и без «другое время/место/дату»: «хочу другое время» —
    # не ответ про питание, а `other` означал бы «и я напишу словами».
    (
        DIET_OTHER,
        r"\bдругое\b(?!\s+(врем|мест|дат))|друг(ая|ую)\s+диет|особ(ое питание|ая диета)",
    ),
    (
        DIET_OMNIVORE,
        # «нет/без ограничений» — в обе стороны порядка слов. Односторонний
        # шаблон уже терял самую простую формулировку («нет ограничений»), и
        # каждая потерянная формулировка — это человек, которого спросят
        # снова через сутки.
        r"(нет|без)\s+(никаких\s+)?ограничений|(ни)?как(их)?\s+ограничений"
        r"|ограничени\w*\s+нет|ем всё|ем все|всё ем|все ем|всеядн",
    ),
)
...
def _parse_diet(text: str) -> Any:
    ...
    for value, pattern in _DIET_WORDS:
        ...
        if (
            value == DIET_OMNIVORE
            and _DIET_EXCLUSION_RE.search(low)
            and not _NO_EXCLUSION_RE.search(low)
        ):
