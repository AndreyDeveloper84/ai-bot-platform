...
CLARIFY_FOOD_ENTRY_TOOL_SPEC: dict[str, Any] = {
    ...
    "description": (
        ...
        "словами. "
        # DRF-2285 (живой проход 22.09): «Сфотографируем еду?» → «Я не умею
        # делать фото». Фото без подписи — рабочий вход в сканер
        # (is_structured_nutrition_turn), но модель об этом не знала.
        "Фото еды человек присылает прямо в этот чат — бот распознаёт его сам "
        "и покажет, прежде чем записать; на «сфотографируем еду?» пригласи "
        "прислать фото, не говори, что не умеешь."
    ),
    "parameters": {
        "type": "object",
        "properties": {
