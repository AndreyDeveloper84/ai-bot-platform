...
_CLIENT_PATH = (
    ...
    # DRF-1491 — главное меню витрины, экран промаха и запрос согласия на
    # медданные. Копия живёт в навыке, а не в канале, но читает её тот же
    # человек и на том же пути, и цена замены здесь выше обычной: у
    # ответа на отказ («больше не спрошу») подмена стоит не только слов.
    # Замена сбрасывает ``action_type`` на ``OUTBOUND_ACTION_TYPE``, а по
    # нему ``handler._health_already_declined`` и узнаёт, что человека уже
    # спрашивали, — то есть сработавший здесь ложный срабатыш вернул бы
    # повторный запрос согласия, запрещённый решением владельца §25 п.6.
    "apps/skills/menu/marketplace.py",
)
...
_BENIGN = (
    ...
    # «Курс из 10 сеансов…» переехал отсюда в _MUST_BLOCK: с P1-D1 это
    # НЕ благой ответ, а ровно то, что планировочная категория обязана
    # ловить. ADR-0012 N-04 прямо запрещает предлагать курс процедур из
    # вывода модели, а §50 фиксирует повторяемость как UNKNOWN — сказать
    # это нам нечем. Прежняя классификация была верна ровно до того дня,
    # когда категория появилась.
    #
    # Планировочные приглашения БЕЗ нормы обязаны остаться благими —
    # именно на них категория и могла бы перестараться:
...
_KNOWN_FALSE_POSITIVES = (
    # «Диагноз может поставить только врач…» ОТСЮДА УШЛА: шаблон диагноза
    # теперь требует связку, и отклоняющая формулировка проходит. Список
    # известных промахов сократился — редкое направление, и ради него
    # правка и делалась.
    "Антибиотики и массаж — вопрос к лечащему врачу, не ко мне.",
    "Точно поможет только регулярность — один сеанс это приятно, но ненадолго.",
)
...
_MUST_BLOCK = (
    ...
    # P1-D1 (§50): планировочная выдумка. До этой категории каждая из
    # четырёх уходила человеку — фильтр их не видел, а в промптах правил
    # про интервалы, курсы, совместимость и восстановление не было ни
    # одного. Запрет «модель не придумывает тайминги» существовал только
    # как намерение.
    ("planning", "Между курсами нужно три-четыре недели."),
    ("planning", "Курс из 10 сеансов обычно берут, чтобы эффект держался дольше."),
    ("planning", "Эту процедуру нельзя совмещать с пилингом."),
    ("planning", "Восстановление занимает три дня."),
)
