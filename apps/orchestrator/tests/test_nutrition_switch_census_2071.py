...
#: DRF-2455 — у снимка записи свой параметр. Перепись обязана уметь
#: построить адрес каждого маршрута питания, иначе новый маршрут выпадает
#: из неё молча — а именно этого она и не должна допускать.
_PATH_ARGS = {"entry_id": "e1", "meal_id": "m1", "log_id": "l1"}
...
#: Уровень навыка: кнопки доезжают на любом боте (структурный префикс),
#: текстовая форма — по лестнице тенантного бота / при активной анкете; на
#: глобальном боте без анкеты свободный текст уходит консьержу (устройство
#: маршрута, не ворот — ``is_structured_nutrition_turn``).
