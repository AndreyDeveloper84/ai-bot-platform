...
class _HonestModel:
    ...
    async def complete(self, messages, *, model, tools=None, **kwargs):
        ...
        return CompletionResult(
            # DRF-1827 — без «покажу»: обещание без вызова инструмента
            # исходящий сторож теперь заменяет честной строкой.
            text="Поняла: спина, после работы. Подойдёт массаж спины и шейно-воротниковой зоны. Смотрим мастеров в Пензе?",
            tool_calls=[],
            prompt_tokens=30,
