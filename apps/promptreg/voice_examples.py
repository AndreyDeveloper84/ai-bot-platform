...
ANKETA_EMPATHY_EXAMPLES: list[Example] = [
    Example(
        user="65",
        assistant=(
            "Принято — 65 кг. Сейчас спрошу про активность, потом всё сразу посчитаю ориентиры."
        ),
    ),
    # DRF-2105 — без числа-нормы темпа («2–3 кг/месяц» владелец нигде не
    # утверждал; §85 задаёт только поправку ≤ 10 % к поддержанию) и без
    # обещания «темпа» (в анкете ``pace`` намеренно не шаг — ``fsm.py``).
    # Скорость снижения — к специалисту; здесь — цель «снижение» и ориентир.
    # Формулировку выбирал владелец из двух (PR DRF-2105).
    Example(
    ...
    # DRF-2104 — §7.1 (решение владельца 09.09): расстройство пищевого
    # поведения — стоп-сценарий. Ориентиры не считаются, анкета не
    # продолжается; «РПП → maintain» снята. Ответ — признание и мягкий стоп
    # в словах детерминированного пути (``nutrition_anketa._STOP_SCREENING``):
    # числа называет специалист, дневник остаётся. Не медицинский совет.
    # Прежний текст («программа поддержания … пройдёмся по анкете») обещал
    # ровно то, чего стоп не делает. Сторож: tests/test_stop_group_examples_2104.
    Example(
