...
-------------------------
Есть: что показано (WHAT, WHY, факты, из которых WHY собран, и какие
ДРУГИЕ подходы стояли рядом), кому, под какую цель, и реакция человека
(B8: ENGAGED доказывает взаимодействие — `accepted` снят). Нет: услуги, мастера, цены, слота — карточка C04 их не
...
class Recommendation(models.Model):
    ...
    #: Другие подходы, ПОКАЗАННЫЕ с этой карточкой (C04.3, DRF-1770): ≤2
    #: строк `{"what", "subline"}` из той же курируемой таблицы. Хранятся,
    #: а не перезапрашиваются: запись immutable (B13), и кадр «что для
    #: тебя важнее» через час должен показывать то же, что показал тогда,
    #: даже если таблицу владельца с тех пор правили.
