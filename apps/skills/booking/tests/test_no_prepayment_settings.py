"""DRF-1007 — ``BOOKING_NO_PREPAYMENT_TENANTS`` settings wiring.

Mirrors ``test_health_gate_settings.py`` (DRF-1005): the setting is
declared in ``config/settings/base.py``, defaults to deny-all (empty
frozenset → ``payment_required=True`` everywhere, behaviour unchanged),
parses a CSV of canonical tenant UUIDs from the environment, and a
malformed value refuses to boot (``ImproperlyConfigured``) instead of
silently becoming an empty allowlist.
"""
...
class TestAttributeDeclared:
    def test_declared_default_deny_all(self) -> None:
        assert dj_settings.BOOKING_NO_PREPAYMENT_TENANTS == frozenset()
...
class TestBaseSettingsParsing:
    def test_unset_env_defaults_to_deny_all(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("BOOKING_NO_PREPAYMENT_TENANTS", raising=False)
        base = importlib.import_module("config.settings.base")
        assert base.BOOKING_NO_PREPAYMENT_TENANTS == frozenset()

    def test_valid_csv_parses(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv(
            "BOOKING_NO_PREPAYMENT_TENANTS",
    ...
    def test_malformed_refuses_boot(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """A typo'd allowlist must fail LOUDLY at settings load — silently
        parsing to empty would flip the pilot tenant back to prepayment
        while the operator believes the no-prepayment list is active."""
        monkeypatch.setenv("BOOKING_NO_PREPAYMENT_TENANTS", "not-a-uuid")
