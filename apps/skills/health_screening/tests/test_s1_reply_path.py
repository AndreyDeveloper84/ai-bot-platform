...
#: DRF-2000 (S-2): группа «неотложка» останавливается на гейте вердиктом
#: ``medical`` — тем же текстом [OD-BOT §163], что и навык, но до навыка и до
#: модели, на всех поверхностях. Одиннадцать фикстур G1–G3; до S-2 гейт
#: держал одну («сердечный приступ»), и та — кризисным текстом.
GATE_STOPS: frozenset[tuple[str, str, str]] = frozenset(
    {
        ("G1", "explicit", "задыхаюсь"),
        ("G2", "explicit", "теряю сознание"),
        ("G2", "explicit", "потеряла сознание"),
        ("G2", "explicit", "только что упала в обморок"),
        ("G2", "post_procedure", "после процедуры потеряла сознание"),
        ("G2", "long", "теряю сознание"),
        ("G3", "explicit", "резко давит в груди и плохо"),
        ("G3", "explicit", "сильная боль в груди"),
        ("G3", "explicit", "сердечный приступ"),
        ("G3", "post_procedure", "после сеанса резко давит в груди"),
        ("G3", "long", "резко давит в груди и плохо"),
    }
)
...
#: 20.09 ([OD-BOT §164] question flow): ambiguous G4 «немеет рука иногда» — вопрос, не
#: red flag → 57 − 1 = 56.
