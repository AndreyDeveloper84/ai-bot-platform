...
Таблица описывает МЕХАНИЗМ, и он в силе. Сегодня она ничего не рисует:
:data:`NUTRITION_ITEMS` пуст с DRF-1543 (ручек ``customer/food/*`` нет),
поэтому вторая и третья строки достижимы только тогда, когда пункты
вернут. Ворота при этом не тронуты — вернуть пункт значит вернуть и обе
строки.
...
#:
#: СЕЙЧАС ПУСТО — DRF-1543, решение владельца 06.09.2026, вариант A.
#:
#: Оба пункта («📸 Сканер еды», «📔 Дневник питания») сняты ПОТОМУ, что
#: за ними нет экрана: ручек ``customer/food/*`` в
#: ``apps/miniapp_api/urls.py`` не существует, и ``guardProd``
#: (``apps/miniapp/src/lib/food-scanner.ts``) на прод-сборке бросает.
#: Обычная заглушка была бы полбеды; здесь ворота §25 п.6 пускают дальше
#: ТОЛЬКО того, кто уже дал согласие ``HEALTH``, — то есть человек
#: отдавал медданные и получал ошибку.
#:
#: ВЕРНУТЬ — вписать записи обратно в этот кортеж, одной строкой на
#: пункт, КОГДА ПОЯВЯТСЯ РУЧКИ ``customer/food/*``. Больше ничего менять
#: не нужно: и клавиатура, и перечень в тексте, и таблица фраз
#: (:func:`health_tap_text`), и разбор ``cb:health:need:{surface}``
#: собираются из этого кортежа.
#:
#: Что НЕ снято и снято быть не должно:
#: * ворота §25 п.6 целиком — механизм ``cb:health:need:{surface}``,
#:   двойная проверка (отрисовка + тап), память отказа. Пустой кортеж их
#:   не ломает: ``health_need_surface`` отвечает ``None`` на любой слаг, а
#:   ``handler._route_health_callback`` на ``None`` уже умеет — это ветка
#:   «снятый пункт», и человек с payload'ом из истории чата получает меню;
#: * слаги ``open_food_scan`` / ``open_food_diary`` в
#:   ``apps.skills.welcome.skill.MINIAPP_ROUTES`` и в ``_ROUTE_MAP``
#:   мини-приложения — снят ПУНКТ МЕНЮ, а не маршрут;
#: * ботовый дневник питания (``apps.skills.food_scanner``) — он живой и
#:   рабочий: скан, уточнение, коррекция, память, «что я ел», вода.
...
#
# Сканер и дневник СНАЧАЛА были оставлены по признаку «отложено —
# снимаем, делается — оставляем»: считалось, что ручки заводятся в
# текущей неделе, а до тех пор человек читает честное «ещё не
# подключён». Замер боевого контура 06.09.2026 этот довод снял: ручек
# ``customer/food/*`` нет, ``guardProd`` бросает, а
# ``FoodScannerProcessingScreen`` вместо честного текста говорит
# «сервис временно недоступен, попробуй через минуту» — то есть врёт о
# природе отказа (второй дефект DRF-1543, правится отдельно).
#
# Поэтому граница проведена по ОДНОМУ признаку для всех трёх пунктов:
# нет живого экрана — нет пункта. Оба пищевых сняты (DRF-1543, вариант
# A, решение владельца); «💚 Самочувствие» снята раньше (DRF-1491) и по
# той же причине — не возвращать. Вернуть пункт — одна запись в
