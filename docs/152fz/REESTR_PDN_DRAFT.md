...
| **Эскалация к человеку** | `AdminTask.transcript_snapshot` (JSON) — **замороженная копия переписки + профиля клиента** на момент эскалации; `reason` (на путях handoff — «Trigger phrase: {реплика}», у жалобы — фрагмент отзыва), `resolution_note` | PostgreSQL | `bot:apps/handoff/models.py:46`, `:108`, `:132`, `:137` | **VERIFIED** |
...
Основание срока строки 90 дней, кто и как читает журнал, что стирает «забудь всё» и где доступ шире, чем нужно, — [`WEBHOOK_JOURNAL_RETENTION.md`](WEBHOOK_JOURNAL_RETENTION.md) (CD §76 №5, на подтверждение владельца).
...
| ~~В модели **нет** полей TTL/срока хранения~~ **Срок введён DRF-2242:** тело → `{}` через `INGRESS_RAW_RETENTION_HOURS` (72 ч, тот же, что у потоков `ingress:*`, DRF-2220); строка без тела удаляется через `WEBHOOK_JOURNAL_ROW_RETENTION_DAYS` (90 дней — решение главного окна по аналогии с ярусом аудита, владельцу вынесено). Периодическая задача `apps.ingress.tasks.sweep_webhook_journal` (ежечасно) берёт только кромку последних 7 дней; накопленное до выкладки — команда `purge_webhook_journal`, сухой прогон по умолчанию, `--apply` по слову владельца | `bot:apps/ingress/retention.py`; расписание `bot:config/settings/base.py` (`ingress.sweep_webhook_journal`) | **VERIFIED** |
| Строка без тела — **псевдонимизированные ПДн**: `trace_id` → `Message.trace_id` → диалог → `BotUser.channel_user_id`; `external_event_id` (`callback_id` / `update_id` / `mid:seq`) можно запросить у MAX. При «забудь всё» и удалении аккаунта у строк человека связь рвётся сразу: тело → `{}`, `trace_id` → `""`, event id → `erased:<sha256>` (дедуп повтора сохраняется) | `bot:apps/ingress/retention.py` (`erase_person_rows`), вызов `bot:apps/conversations/erasure.py` (`_erase_journal_rows`) | **VERIFIED** |
| В персональную выгрузку журнал **не входит** намеренно: тело — техническая копия тех же реплик, что уже выгружаются как сообщения, и живёт ≤ 72 ч; строка без тела — служебный след без содержания | `bot:apps/identity/services/privacy.py` (`export_personal_data`) — раздела журнала нет | **VERIFIED** |
...
|---|---|---|---|
| ~~**`WebhookJournal.raw_payload`**~~ **закрыто DRF-2242** (см. 2.4.1): тело ≤ 72 ч, строка ≤ 90 дней. Ниже — прежняя запись. Сырые вебхуки с именами, текстами сообщений, возможно телефонами | **Механизма очистки НЕ НАЙДЕНО.** В `apps/ingress/` нет `tasks.py`, нет management-команд, нет записи в расписании celery beat, нигде нет `WebhookJournal…delete()`. Хранилище растёт неограниченно | `bot:apps/ingress/models.py:25`, `:37-40`; отсутствие — по всему `bot:apps/ingress/` и `bot:config/settings/base.py:948+` | **VERIFIED (отсутствие)** |
...
| ~~`WebhookJournal.raw_payload` — сырой вебхук с именем и текстом~~ **закрыто DRF-2242:** шаг `_erase_journal_rows` в `anonymize_dialogue` — по телу (отправитель) и по `trace_id` сообщений человека; тело `{}`, trace `""`, event id хешируется | `bot:apps/conversations/erasure.py` | **VERIFIED** |
| ~~`AdminTask.transcript_snapshot` — замороженная копия переписки~~ **закрыто DRF-2243:** шаг `conversations.erasure._anonymize_admin_tasks` внутри `anonymize_dialogue` (его зовут и «забудь всё», и каскад удаления) — `messages[].content`, `display_name`, `channel_user_id`, `phone_hash` → `""` по тому же cutoff; `reason` со свободным текстом («Trigger phrase: …», фрагмент отзыва) → `""`, код причины остаётся; строка задачи, тип, статус, время — остаются. **Не затрагивается:** `resolution_note` — текст оператора | `bot:apps/conversations/erasure.py` (`_anonymize_admin_tasks`); тест `bot:apps/conversations/tests/test_admintask_snapshot_erasure_2243.py` | **VERIFIED** |
...
| 8 | ~~**`WebhookJournal.raw_payload` копится бессрочно**~~ **закрыто DRF-2242** (тело ≤ 72 ч, строка ≤ 90 дней, «забудь всё» рвёт связь; накопленное — командой по слову владельца). Прежняя запись: **`WebhookJournal.raw_payload` копится бессрочно** — сырые вебхуки с именами, полным текстом сообщений, возможно телефонами; очистки нет; поле выведено в админку | **ВЫСОКИЙ** | `bot:apps/ingress/models.py:37-40`; `bot:apps/ingress/admin.py:31`; отсутствие задачи очистки — по всему `bot:apps/ingress/` |
...
| ~~P1~~ | ~~Ввести retention для `WebhookJournal`~~ — **сделано DRF-2242** (`bot:apps/ingress/retention.py`); `--apply` по накопленному — за владельцем | `bot:apps/ingress/models.py:25` |
