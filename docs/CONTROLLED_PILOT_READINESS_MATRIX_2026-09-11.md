...
UNKNOWN_NOT_MEASURED COUNT:   5 → 5           (шесть запрошенных чисел сняты 14:47–14:50 UTC; одно осталось)
```
...
| 10 | Observability инцидента | AIDailyMetricSummary 0, ReplayTrace 0, id не переживает границу | **не изменилось — подтверждено данными 14:50 UTC**: `AIRequestMetric` 268 / `AIDailyMetricSummary` 0 / `ReplayTrace` 0; ящик 53 события, 53 недоставлено, попыток 0, последняя 08.09 11:01; `recommendation*`/`safety*`/`stale_callback` — 0 строк из 24 166 | `MEASURE_MATRIX_INPUTS_2026-09-11.md` |
...
Выложенные SHA (снято на хосте 14:47 UTC): бот `005260b3` (deploy-dev 34607527799, 14:05 UTC); каталог
`3a0bc0d` (17:10+03) — **пачка #330…#352 на пилот ещё не выехала** (test красный по двум листам
`0018`, #361 merge-миграция в сливе). Значит база замера каталога (`98d4fada`) **впереди пилота**:
всё, что в матрице помечено «слито» по каталогу после 14:10 UTC (#345 цели, #324 граница питания,
#342 геокодер, #346 сброс, #339, #330, #354), на пилоте на момент снятия чисел **не исполняется**.
Числа с пилота (`MEASURE_MATRIX_INPUTS_2026-09-11.md`, 14:47–14:50 UTC) — с `3a0bc0d`.
...
Срез `formula-tela` (14:47 UTC): SalonService **58 — все `unmapped`** (review_required 0, verified 0),
SpecialistService 232; `requires_health_check = true` у SalonService — **1 на весь контур** (09.09: та
же одна, демо-салон), у ServiceTemplate — 102. Число `resolved_requires_health_check = None` не
снято (свойство вычисляемое, не колонка) — §29.
...
**Пилотный салон: 0 услуг с гейтом, 58 из 58 `unmapped` = без шаблона** (14:47 UTC). Читать как
«безопасно» по-прежнему нельзя — но теперь ноль защищён механизмом: первая же услуга с гейтом
упрётся в `check_health_screening` на любом входе; у 58 без шаблона `resolved_requires_health_check`
по коду #307 должен быть `None` → fail-closed, число живьём не снято (свойство, не колонка).
...
**По коду — не изменилось ни в одном пункте B-7** (§6), **и данные это подтверждают** (14:47–14:50 UTC,
`MEASURE_MATRIX_INPUTS_2026-09-11.md`):

```
бот   AIRequestMetric 268 (09.09: 266) | AIDailyMetricSummary 0 | ReplayTrace 0
      eventbus_domainevent (ящик): 53 всего, 53 недоставлено, attempts>0 — 0, dead-letter 0,
                                   последняя 08.09 11:01 — диспетчер не ходит вовсе
      events.Event 24 166 (09.09: 21 930), последняя 14:46; marketplace.booking_callback.routed 55,
                   booking_confirmed 29, booking_created 25, booking_cancelled 19, booking_lookup.routed 5
                   recommendation* 0 | safety* 0 | stale_callback 0
Ayla  AnalyticsEvent: goal_selected 36, external_identity_bound 1 — всё (как 09.09)
```

`+2 236` событий за двое суток при 0 клиентских ходов — собственная синхронизация и коллбэки, как и
09.09 (96 % трафика — синхронизация).
...
```
tenant формула-тела (14:47 UTC): 58 SalonService — unmapped 58 / review_required 0 / verified 0
  SpecialistService 232 (09.09: 232, активных 95 — сегодня не разделялось)
  safety: гейт поднят у 0 (на весь контур — 1, демо-салон); шаблонов с гейтом 102 из 1223
```

**PILOT RECOMMENDATION SUBSET = 0**, как 09.09. Путь к первому `VERIFIED` теперь оформлен механизмом
...
AI-чат каталога под трафиком; Google Doc KB). Шесть чисел, запрошенных 14:45 UTC, **сняты главным
окном 14:47–14:50 UTC** (`MEASURE_MATRIX_INPUTS_2026-09-11.md`, только чтение) и вписаны в §1, §4,
§10, §19, §24. Одно осталось не снятым: число единиц с `resolved_requires_health_check = None`
после #307 — свойство вычисляемое, а не колонка; снимать надо командой каталога, не SQL.

**Новое белое пятно, найденное этими числами:** база замера каталога (`98d4fada`) **впереди
выложенного** (`3a0bc0d`): пачка 14:30 на пилот не выехала (test красный по `0018`, #361
merge-миграция). Всё «слито» по каталогу после 14:10 UTC в этой матрице — состояние `dev`, не
пилота; для владельца это значит, что вердикт по коду опережает вердикт по машине на одну
выкладку.
