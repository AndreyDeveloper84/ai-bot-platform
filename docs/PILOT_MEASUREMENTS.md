...
**Разделение источников.** Живые числа §11.0 сняло главное окно из контейнера бота
09.09.2026 — у окон-исполнителей `ssh` закрыт. Всё остальное — замер **кода и
конфигурации**, сделанный этим окном; он воспроизводится без доступа к пилоту и потому
не протухает вместе с числами.

## 11.0 Содержимое ящика — живой замер

**Снято** 09.09.2026 главным окном, контейнер бота.

```
TOTAL       53
DISPATCHED   0
PENDING     53
OLDEST      12.08.2026 05:47 UTC   — двадцать восемь суток
NEWEST      08.09.2026 11:01 UTC

booking.created                16
booking.attribution.assigned   16
booking.completed               8
customer.consent.changed       13
```

**Что это значит.** Ящик копится **двадцать восемь суток** и не разобран ни разу:
`DISPATCHED = 0` при `TOTAL = 53`. Ноль отправленных здесь не «всё доставлено», а
«никто не пробовал» — попыток тоже ноль, потому что пробовать некому (§11.3).

Тем же замером закрыты два пункта, стоявшие в «не замерено»: контейнер `celery-beat`
**идёт** (Up 9 hours, `DJANGO_SETTINGS_MODULE=config.settings.staging`) — то есть три
чистильщика приёма работают, и дело именно в отсутствующей записи расписания, а не в
мёртвом контейнере; переменная `DOMAIN_EVENT_SUBSCRIBERS` **не задана ни в одном
процессе** (`celery-worker`, `web`) — значит держится умолчание настроек, и подписчик
действительно `NoopSubscriber` (§11.4).
...
## 11.7-бис Тринадцать из пятидесяти трёх — это согласия, и вес у них другой

`customer.consent.changed` — 13 записей из 53. Их стоит считать отдельно от остальных
сорока: те про записи и атрибуцию, эти про то, кто разрешил и кто отозвал.

**Чего НЕ произошло — сказать это первым, чтобы не пугать зря.** Само согласие не
потеряно. На каждом изменении `apps/consent/services.py` пишет его **тремя** независимыми
способами, и все три коммитятся до шины:

1. строка `ConsentRecord` — первичная запись;
2. аналитическое событие `CONSENT_GRANTED` (`apps.events.Event`);
3. локальный аудит `write_audit("consent.granted"/"consent.withdrawn")`.

Шина — **четвёртая** копия, и застряла именно она. Отзыв согласия исполняется по строке
`ConsentRecord`, а не по событию, поэтому «человек отозвал, а система не узнала» здесь
не произошло.

**Что произошло.** Доменный след изменений согласий за двадцать восемь суток не покинул
ящик, а в нём — и выдачи (`granted=True`), и отзывы (`granted=False`: тот же тип события,
флаг и есть бит жизненного цикла, `services.py:344-358`). Предполагаемый потребитель —
`AuditSubscriber`, и он написал бы ещё одну строку аудита; на пилоте его нет (§11.4).

**Почему это всё же отдельная строка, а не сноска.** Если починку сделать в наивном
порядке, эти 13 будут помечены доставленными никому — вместе с остальными сорока. Но
цена у них другая: следующий, кто построит потребителя согласий на шине (распространение
отзыва во внешнюю систему — ровно тот случай, ради которого доменная шина и заводится),
начнёт с пустой историей и не узнает, что она была непустой. Сорок событий об атрибуции
переживут такую потерю; тринадцать событий о разрешениях — вопрос 152-ФЗ, и решать его
...
Без него об исходе можно будет сказать только «стало ноль» — а ноль даёт и `Noop`.

**Порядок зафиксирован главным окном 09.09: счётчик → подписчик → расписание.**
Любой другой порядок стоит права на 53 накопленные строки (§11.5).
