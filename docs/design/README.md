...
- **ai-quality-observability.md** — Owner + founder dashboard for monitoring AI quality (distinct from analytics-dashboard booking/revenue KPIs). Tracks persona violation rate (target <2%), CSAT (target ≥4.5★), handoff rate, response time, model version drift. Sampling logic (5% random + auto-flag rules), conversation review queue, per-template performance breakdown, model deploy delta tracking. **Founder-50 cohort review workflow** (Q12-δ implementation) — first 50 bookings per tenant manually reviewed before auto-billing enables. Alert thresholds + sub-pages for Violations / Templates / Models / Reviews / A/B (Phase 2+). Privacy: customer never sees flagged status; cross-tenant data founder-only
- **customer-profile-management-ux.md** — Mini App «Профиль» tab: 6 sections (header card + Мои записи + Самочувствие + Уведомления + Приватность и данные + Помощь). Visit history with cancel/reschedule deeplinks, wellness module activation panel, notification preferences shortcut, **OP6 deletion request flow** (30-day grace + admin verification + warning modal), data export request (rate-limited 1/90d, MAX delivery MVP), block-bot path. Section 7 Help with FAQ + «о помощнике» honesty modal. 18 open questions Q-CP1-18
- **tenant-suspension-pause-ux.md** — 5-state tenant lifecycle (ACTIVE → AT_RISK_BILLING → PAUSED → SUSPENDED → ARCHIVED) with per-state per-audience UX (customer / master / owner / admin). Billing failure handling (7d grace), owner-initiated voluntary pause, recovery flow (self-service + CSM-mediated), API endpoint disablement matrix per state, notification cascade with throttling, data continuity rules per [Q-C3 retention](../decisions-log.md), 15 edge cases incl. mid-conversation/mid-booking transitions, anti-patterns (no shame language, no customer punishment, no auto-restore zombie bookings). 17 open questions Q-TS1-17
...
- **2026-05-19-wellness-ai-avatar-handoff.md** — Phase 3 strategic «WOW» retention feature (`apps/wellness_avatar/`). Independent app from mood module for scope isolation. 4 models: AvatarConsent + AvatarZoneConsent + AvatarPhoto + AvatarComparison + AvatarMasterGrant. 6 zones (face / body / waist / hips / nails / hair) with **per-zone consent** + privacy-paranoid client-side AES-GCM encryption (server never sees plaintext). Multi-step consent wizard (intro + zone selection + confirmation). Photo capture + comparison view + AI commentary with **honest fallback** (NEVER fakes improvements, NEVER generates futures, NEVER body-shames). Master-grant flow (customer grants per-master per-zone time-bound access; audit 7y). 11 API endpoints. Privacy hierarchy stricter than other modules (24h key purge vs 30d for others). Founder breakglass via legal hold + 4-eye approval only. Q-AV1-18
