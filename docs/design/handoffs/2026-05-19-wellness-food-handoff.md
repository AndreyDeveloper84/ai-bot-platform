...
Per [`project_ayla_first_strategic_pivot`](../policies/ayla-identity-and-brand.md) memory 2026-05-19: food data is **Ayla's memory of user** per [`ayla-memory-and-personalization §9`](../policies/ayla-memory-and-personalization.md) — cross-tenant. Eating-disorder silent mode + pregnancy override + BMR floor preserved as core safety nets. NO «-N kg target» anti-pattern intact. `HUMAN_LOCKED` references — backend mechanic; customer-facing escalation via [`ayla-emergency-fallback-policy`](../policies/ayla-emergency-fallback-policy.md). AI voice uses Ayla per [`ayla-identity-and-brand §2`](../policies/ayla-identity-and-brand.md).
