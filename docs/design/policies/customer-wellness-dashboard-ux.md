...
Per [`project_ayla_first_strategic_pivot`](./ayla-identity-and-brand.md) memory 2026-05-19: wellness data is **Ayla's memory of user** per [`ayla-memory-and-personalization §9`](./ayla-memory-and-personalization.md) — cross-tenant persistent (customer's wellness follows them across all salons). Tenant CANNOT see this data per [`tenant-as-provider-model §4.4`](./tenant-as-provider-model.md). All AI voice samples in this doc use «Ayla» as first-person actor per [`ayla-identity-and-brand §2`](./ayla-identity-and-brand.md).
