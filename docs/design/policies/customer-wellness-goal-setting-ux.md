...
#### 2.4.1 Именованное исключение: недельная оглядка — дни дневника и дни цели

**Решения владельца 07.09.2026, дословно:** «не снимаем» → «§2.4 получает
именованное исключение для этого блока, и он строится как нарисован —
„4 из 7 дней"» → «нам нужны и дни цели и дни дневника» → **«днём цели
считается каждый день после выбора цели»**.

Tau Block 6 «Прогресс недели» строится и охватывает обе величины. Но
определение дня цели, данное владельцем, делает эти величины **разными по
устройству**, и рисовать их одной строкой нельзя.

**Дни дневника — счёт с провалом.** «Вода: 4 из 7 дней» — дни, в которые
велась запись, из семи. Пропуск виден. Это и есть форма, которую 2.4 называет
по имени, и **именно она пользуется исключением**.

**Дни цели — счёт без провала.** Днём цели считается **каждый день после
выбора цели**: счётчик растёт сам, независимо от того, делал человек что-либо
или нет. Пропустить день **невозможно**, значит наказывать нечем — и
серией в смысле 2.4 это не является. Исключение для этой половины
**не требуется**; она проходит по 2.4 без изъятия.

Отсюда форма: у цели знаменатель — **не неделя, а срок**. «День 12 из 30»,
а не «7 из 7»: при таком определении недельная дробь у цели всегда полная и
ничего не сообщает.

**Две механики цели, обе названы владельцем.**

| механика | что человек задаёт | что показываем |
|---|---|---|
| **срок** | начало запуска и продолжительность | «день N из M» |
| **событие** | дата события | сколько дней до события |

Про вторую владелец сказал прямо: **количество дней на результат не влияет**,
механика другая. Значит для события **не рисуется ни дробь, ни доля** — только
расстояние до даты.

**Чего для этого не хватает в данных.** Начало есть — `ClientGoal.selected_at`.
**Продолжительности нет**, поля под неё не существует. Дата события живёт **не
на `ClientGoal`**: она на `DesiredOutcome`, горизонт — на `PlanOutcomeLink`
(`OPEN_DECISIONS` §54). То есть обе механики требуют полей, которых сегодня нет,
и **выбор срока человеком** — отдельный экран, которого тоже нет.

Под исключение **не подпадают** и остаются запрещёнными:

* «дни без срыва» и любая формулировка, называющая пропуск срывом;
* счётчик визитов (`OPEN_DECISIONS` §35 п.13; визиты — серия) — и считать его
  нечем: на пилоте **ноль завершённых визитов**;
* понукание по 2.5 — оглядка показывается, когда человек пришёл, и не
  рассылается сама;
* перенос формы «N из 7» на другие экраны без нового решения владельца.

**Порог тишины — только для половины дневника.** Блок молчит, пока за неделю
нет ни одной записи: «0 из 7» каждую неделю показывает человеку не прогресс,
а его собственную пустоту. Численный порог — цифра владельца; до её назначения
действует минимальный: **хотя бы одна запись за неделю**. К дням цели порог
не применяется: там нуля не бывает по определению.

**Источника сегодня нет.** `RecentActivity.weekly_progress` объявлен в
контракте, но ручки списка приёмов пищи не существует (DRF-1476). Исключение
записано заранее, чтобы исполнитель, у которого источник появится, не отверг
задание по 2.4 и не построил его в обход решения владельца.
