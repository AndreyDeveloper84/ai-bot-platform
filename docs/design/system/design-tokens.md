...
Ролей в коде восемнадцать, подписей на борде восемь. Разницу закрывает одно
правило, применённое механически и одинаково в обеих темах:

> смысл цвета, тон (H) и максимально возможная насыщенность (S)
> подписанного значения сохраняются; светлота (L) двигается **ровно
> настолько**, сколько нужно, чтобы дать **4.5:1** к худшей поверхности
> своей темы.

**Результат сдвига — отдельный токен, а не замена подписи** (решение
владельца §21-quinquies п. 5 от 09.09.2026). Подписанное значение остаётся
в `--c-<роль>` и красит фон, иконки и декор; сдвинутое живёт рядом в
`--c-<роль>-text` и красит только текст. Отсюда `--c-success` `#22C55E` и
`--c-success-text` `#157A3A` — разные роли, а не два имени одного цвета;
одного зелёного или жёлтого на все случаи в палитре нет.

Дословная формулировка владельца: «Нужны отдельные токены, например
`success-text`, `warning-text`, `danger-text`, а не один зелёный или жёлтый
на все случаи».
...
не проходит AA для основного текста, промахиваясь на 0.19. Плюс `#6B7280`
даёт 4.39:1 на вторичной поверхности. У sage такая работа была проделана и
записана; у фиолетового её не было, и §21-ter п. 2 прямо относит её к
объёму задачи, а не к следующей.

**Основной цвет — один: `#4452FF`** (§21-quinquies п. 3). Он же интерфейс,
он же клиентский аватар, он же основные кнопки и активные состояния.
`#7D63EF` как второй фиолетовый убран; мастерского бота по-прежнему
отличаем тёмным фоном, но его акцент происходит от того же `#4452FF`.

`#9DA5FF` в тёмной теме — **не конкурент `#4452FF`, а его вторая светлотная
форма**. Тон совпадает (H≈235.3), насыщенность совпадает (S=100 %),
различается только светлота: 63.3 % против 80.8 %. Связь названа здесь и в
комментарии у токена именно потому, что прочитав только «канон — `#4452FF`»,
легко подставить его в тёмную тему: на `--c-surface-2` #374151 он даёт
**1.91:1**. Один канонический цвет, две светлотные формы по теме.
...
| `--c-text-on-status` | подписанный «Текст» в обеих темах: яркий статус светел всегда |
| `--c-border-interactive` | H и S «Границы», L двинута до 3:1 к каждой поверхности обеих тем |
| `--c-accent-pressed` | основной + 18 % чёрного (светлая) / белого (тёмная) |
| `--c-accent-subtle` | основной 12 % на белом (светлая) / 18 % на фоне (тёмная) |

### 2.3 · Тёмная тема — канон с 09.09.2026

Тёмной темы нет ни на одном из 24 бордов: она была **выведена** исполнителем
и вынесена владельцу вопросом. **Владелец её утвердил** (§21-quinquies п. 1),
и с 09.09.2026 она часть канона, а не приложение к нему. Довод владельца:
MAX тему не передаёт, поэтому ориентир — системная настройка устройства,
то есть `prefers-color-scheme`.

Утверждены ровно четыре значения:
...
Остальное выведено по правилу §2.2 плюс одно дополнительное:
...
Это ровно то, что владелец и утвердил четвёртой строкой таблицы выше.
...
| `--c-success-text` | `--c-bg` | 5.18:1 | 9.03:1 | ✅ обе |
| `--c-success-text` | `--c-surface-1` | 5.42:1 | 7.47:1 | ✅ обе |
| `--c-success-text` | `--c-surface-2` | 4.92:1 | 5.24:1 | ✅ обе |
| `--c-warning-text` | `--c-bg` | 5.42:1 | 9.27:1 | ✅ обе |
| `--c-warning-text` | `--c-surface-1` | 5.67:1 | 7.67:1 | ✅ обе |
| `--c-warning-text` | `--c-surface-2` | 5.16:1 | 5.39:1 | ✅ обе |
| `--c-danger-text` | `--c-bg` | 5.62:1 | 9.33:1 | ✅ обе |
| `--c-danger-text` | `--c-surface-1` | 5.88:1 | 7.72:1 | ✅ обе |
| `--c-danger-text` | `--c-surface-2` | 5.34:1 | 5.42:1 | ✅ обе |
...
| `--c-text-on-status` | `--c-success` | 7.79:1 | 7.79:1 | ✅ обе |
| `--c-text-on-status` | `--c-warning` | 8.26:1 | 8.26:1 | ✅ обе |
| `--c-text-on-status` | `--c-danger` | 4.71:1 | 4.71:1 | ✅ обе |
| `--c-success-text` | подложка 10 % `--c-success` на `--c-bg` | 4.78:1 | 7.74:1 | ✅ обе |
| `--c-success-text` | подложка 10 % `--c-success` на `--c-surface-1` | 4.97:1 | 6.26:1 | ✅ обе |
| `--c-success-text` | подложка 10 % `--c-success` на `--c-surface-2` | 4.54:1 | 4.52:1 | ✅ обе |
| `--c-warning-text` | подложка 10 % `--c-warning` на `--c-bg` | 5.05:1 | 7.95:1 | ✅ обе |
| `--c-warning-text` | подложка 10 % `--c-warning` на `--c-surface-1` | 5.25:1 | 6.44:1 | ✅ обе |
| `--c-warning-text` | подложка 10 % `--c-warning` на `--c-surface-2` | 4.80:1 | 4.63:1 | ✅ обе |
| `--c-danger-text` | подложка 10 % `--c-danger` на `--c-bg` | 4.95:1 | 8.59:1 | ✅ обе |
| `--c-danger-text` | подложка 10 % `--c-danger` на `--c-surface-1` | 5.15:1 | 7.06:1 | ✅ обе |
| `--c-danger-text` | подложка 10 % `--c-danger` на `--c-surface-2` | 4.71:1 | 5.12:1 | ✅ обе |

Отдельная таблица — **интерактивная граница**, у которой порог другой:
WCAG 2.2 **1.4.11**, 3:1, потому что это граница компонента, а не текст.

| Токен | На фоне | Светлая | Тёмная | 3:1 |
|---|---|---|---|---|
| `--c-border-interactive` | `--c-bg` | 3.23:1 | 5.25:1 | ✅ обе |
| `--c-border-interactive` | `--c-surface-1` | 3.38:1 | 4.35:1 | ✅ обе |
| `--c-border-interactive` | `--c-surface-2` | 3.07:1 | 3.05:1 | ✅ обе |
| `--c-border-decorative` | `--c-surface-1` | 1.24:1 | 1.94:1 | — порога нет, см. ниже |

### 2.4-bis · Две границы вместо одной

Владелец ответил на вопрос §21-quater №2 (решение §21-quinquies п. 2):
интерактивные границы поднимаются до 3:1, а токен разделяется надвое.

| Токен | Значение | Где применяется |
|---|---|---|
| `--c-border-decorative` | `#E5E7EB` / `#4B5563` | **только необязательные разделители**: линейки между рядами, полосы шапок и подвалов, рамки статичных контейнеров, дорожки прогресса, «грипы» шторок |
| `--c-border-interactive` | `#838C9F` в обеих темах | рамка, помогающая понять область нажатия: кнопки, поля ввода, чипы, ячейки слотов, нажимаемые карточки |

Дословно: «Если карточка нажимается и её граница помогает понять область
нажатия, используется `border-interactive`. Да, карточки станут заметнее —
это принимаем».

**Как делили 127 мест.** Признаком служило не имя класса, а элемент, на
котором класс висит: полная рамка на `button` / `input` / `textarea` /
`select` / нажимаемой карточке — интерактивная; односторонняя линейка на
контейнере — декоративная, даже если внутри контейнера есть кнопка. Так
получилось **38** интерактивных мест и **82** декоративных (плюс несколько
упоминаний в комментариях). Разделение по имени дало бы другой ответ:
`.cta-bar` и `.ayla-compose` содержат кнопки, но их `border-top` — это
линейка над панелью, а не её область нажатия.

**Почему значение одно на обе темы.** На фоне `#111827` граница обязана
быть **светлее** фона, а на белом — темнее его, так что «более тёмный
нейтральный» — правило только для светлой темы. Полоса светлот, где одно
значение проходит 3:1 разом на самой тёмной светлой поверхности (`#F3F4F6`)
и на самой светлой тёмной (`#374151`), существует, но узка: L от 56.3 % до
57.4 %. `#838C9F` стоит в её середине. Тон и насыщенность взяты у `#E5E7EB`
(H=220, S=13 %), чтобы холодный уклон ряда `gray` сохранился.

**Ни одна ступень ряда `gray` в эту полосу не попадает** — поэтому взят
сосед не из ряда, а с той же линии H/S: `#9CA3AF` даёт на светлой 2.31:1,
`#6B7280` на тёмной — 2.13:1. И проверка против одного `#ffffff` здесь
обманывает: `#949494` даёт на белом 3.03:1 и всё равно проваливается —
2.76:1 на `--c-surface-2`. Порог берётся против **каждой** поверхности
темы, и сторож считает именно так.

Прежнее ограничение снято: `--c-divider` давал 1.24:1 и единственной
границей интерактивного элемента быть не мог. Теперь такой границы просто
нет — у неё своя роль и свой порог.

**Цветной текст на подложке своего же цвета.** Семь правил `globals.css`
(`.callout--danger`, `.callout--success`, `.m6-bubble--failed`,
`.unbookable-badge`, `.m-card__chip--warning`, `.admin-chip--warn`,
`.m-notif__banner-warning`) кладут цветной текст на `color-mix` из него
самого. Доли были 14 %, 18 % и 20 %; все приведены к **10 %**. Иначе AA
пришлось бы добирать затемнением самих токенов, и основной ушёл бы от
подписи вдвое дальше — дешевле сделать подложку светлее, чем увести цвет
владельца.

После разделения статусов подложка мешается из **яркого** токена, а текст
берётся из `-text`, — и это сложнее прежнего, где обе половины были одним
затемнённым цветом. `.callout--success` этот переход и поймал:
`#16803D` на подложке из `#22C55E` даёт 4.20:1, и `--c-success-text`
пришлось сдвинуть дальше, до `#157A3A`. Пара `success` заодно добавлена в
сторож — форму она имела всегда, а кусаться начала только сейчас.

**Текст на сплошной заливке статуса — тёмный.** Яркий статус светел в обеих
темах, поэтому шесть мест (`.btn-danger`, `.master-tabbar__badge`, три
полосы «офлайн», `.profile-header__avatar`) переведены с
`--c-text-on-accent` на `--c-text-on-status` `#111827`. Белое на `#F59E0B`
давало 2.15:1.
...
| `warm-200` / `--hairline` | `--c-border-decorative` | разделители; для рамки нажимаемого — `--c-border-interactive` |
| `warm-300` / `--track-empty` | `--c-border-decorative` | |
...
| `amber-100` / `amber-700` | `--c-warning` (подложка 10 % / заливка) и `--c-warning-text` (текст) | |
| `rose-100` / `rose-600` / `rose-700` | `--c-danger` (подложка 10 % / заливка) и `--c-danger-text` (текст) | |
| `sage-700` (текстом) | `--c-success-text` | яркий `--c-success` остался для фона и иконок |
| `--paper` | `--c-bg` | |
...
> **Ответ владельца 09.09.2026 (§21-quinquies п. 3).** Два фиолетовых сводятся к одному, и канон — **`#4452FF`**: интерфейс, клиентский аватар, основные кнопки, активные состояния. `#7D63EF` убран. Мастерского бота по-прежнему отличаем тёмным фоном, но его фиолетовый акцент **тоже происходит от `#4452FF`**.
>
> **В токенах это уже исполнено; в ассетах — нет.** Смена `#7D63EF → #4452FF` в клиентском аватаре означает **перевыпуск картинок**, и это отдельная работа. Затронутые файлы (ни один из них здесь не изменён):
>
> * `docs/design/assets/logo/logo.svg` — мастер-вектор, пять вхождений `#7D63EF`;
> * `docs/design/assets/logo/logo.png` — мастер-растр;
> * `docs/design/assets/AYLA/logo.png` и `logo-1.png` … `logo-41.png` — 42 растровых варианта;
> * `docs/design/assets/AYLA/README.md`, `docs/design/assets/AYLA/EXPERIMENTS/README.md`, `docs/design/assets/AYLA/EXPERIMENTS/logo-sage-render.html` — описания, ссылающиеся на старое значение;
> * `docs/design/assets/AYLA/EXPERIMENTS/logo-sage-experiment.svg` — эксперимент, не канон.
>
> Таблица контрастов ниже посчитана против `#7D63EF` и после перевыпуска станет неверной: белый вордмарк на `#4452FF` даёт **5.39:1** вместо 4.51:1, то есть станет лучше, но пересчитать её обязан тот, кто перевыпустит ассеты.
>
> `#7B6CF6` в `Ayla/djangoproject/DESIGN.md` — **другой репозиторий**, отдельная задача, здесь не трогается вовсе (п. 4).
