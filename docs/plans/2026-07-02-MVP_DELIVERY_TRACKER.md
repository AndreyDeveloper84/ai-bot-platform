...
> **✅ v1.5 (2026-07-09): Option B — память для пилота В BOT.** ADR-0009/0011/0006 + построенная bot-система (apps/identity) перевесили «Ayla owns all». Ayla M-A1/A2/A4 закрыты (bot имеет зоны/шифр/red-log/minor); Ayla держит declared-prefs+A1a(done). BOT-скоуп = ACTIVATE: подключить готовые memory_reader/writer к concierge (surfacing read→inject + should_ask→write) — storage готов, в разговор не wired. Стоимость Option A (переписать в Ayla) была ~50-80 SP re-solving; B ≈ проводка M-B ~10-13 SP. «Ayla=central memory» → post-pilot миграция.
> > **✅ G-CalendarSync CLOSED (Variant B, #1103):** double-booking guard = YClients webhook→Ayla busy (Penza на YClients) + recheck-at-confirm (S3-CAL .1/.2/.4 merged #208 за флагом EXTERNAL_BUSY_ENABLED OFF; .3 webhook-ingress ждёт лицензию 884045). Variant A (Ayla-primary) post-pilot; busy-источник абстрагирован → pivot = флаг off. **Открытые хвосты:** (1) Bot→Salon (Ayla→YClients) обратное направление — подтвердить у Ayla-стороны до пилота (#1103 acceptance); (2) walk-in #1017 под Variant B (deferrable?); (3) юрист #947 может флипнуть consent A→B.
...
| S0C.1 | contract tests vs Ayla route-table | bot | #1087 | agent-s0b | 8 | W2 | M | ✅ Done (PR #1087 — **G-Contract ✅**) |
| S0C.2 | обновить contract docs/ADR | bot | #1050 | agent-s0b | 3 | W2 | M | ✅ Done (PR #1087) |
...
| S05.5 | double-contact + MasterNotificationPrefs dispatcher (G-Notify) | bot | #1057 | — | 5 | flip-window (post-Wave-2) | D | **Deferred** (не pilot-crit; notify_channels — cross-repo с Ayla на flip; scope-разбор done) |
