# MVP_GAP_MAP_2026-07 — v1.5
# MVP_GAP_MAP_2026-07 — v1.4

> **v1.5 (2026-07-09) — Option B: память для пилота В BOT.** Reading ADR-0009 §Memory model + ADR-0011 + ADR-0006 показало: ядро inferred-памяти = bot `apps/identity` (УЖЕ построено: UPC/MemoryEntry/RedZoneAccessLog+writer/reader+Fernet+minor-protection+spec+policy+tests, lawyer-pending). «Ayla owns all» (03.07) отменено → post-pilot видение (Ayla=central memory-сервис для многих шлюзов). Ayla M-A1/A2/A4 закрыты (#1094/1095/1097 — bot имеет); Ayla держит declared-prefs+A1a. BOT = ACTIVATE (проводка concierge surfacing+ask поверх готового хранилища; сейчас в разговор НЕ подключено — discovery.py). Ров = возможность памяти (в bot), не репо.
> **v1.4 (2026-07-04) — Stream 5 разблокирован (§8 закрыт):** Fernet-шифрование · green под общим PERSONAL_DATA (формулировка→юрист #947) · fill-rate observe-only · один ayla_user_id на MAX-юзера (телефон=merge-ключ; per-tenant merge post-pilot). Дата **мягкая**; **2-й Ayla-агент** (agent-1=каталог #1044 full-domain параллельно, agent-2=память M-A). Каталог пилот-нить = direct-seed 1 салона в настоящие модели, полный домен без гейта на пилот. Тикеты EPIC #1093 / M-A #1094-1097 / M-B #1098-1100 / M-C #1101.
