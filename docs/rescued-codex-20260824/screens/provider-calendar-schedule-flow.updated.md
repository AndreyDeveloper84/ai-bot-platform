# Provider Calendar / Schedule Flow — Developer Handoff Package

| Field | Value |
|---|---|
| **Date** | 2026-05-28 r1 |
| **Designer** | UX-architect / founder review |
| **Status** | Final draft for Tech Lead review — updated for Ayla auto-reschedule UX |
...
| **Scope** | Provider-side calendar and schedule flow: day/week view, booking list, manual booking, Ayla auto-reschedule updates, time block, working hours, day-off, sick day, vacation, stale slot protection |
...
- Ayla auto-reschedule can move bookings, so calendar must clearly update old/new slots and show provider notification.
...
- stale slot warning.
- Ayla auto-rescheduled a customer booking.
...
### Ayla auto-reschedule calendar update

When Ayla successfully reschedules a booking:

```text
old slot becomes free
new slot becomes occupied
booking card moves to new time
old time may show short “освободилось после переноса” marker until refresh
```

Provider does not approve the eligible reschedule, but must see the result.

Provider-facing copy:

```text
Ayla перенесла запись Анны С.
Было: 16:00
Стало: 18:00
```

If provider is viewing the affected date, update schedule after refetch or realtime event.
...
- Ayla auto-reschedule completed.
...
### 20.1b. Ayla auto-rescheduled while provider calendar was open

If Ayla moved a booking while provider calendar was open:

```text
Расписание обновилось.
Ayla перенесла запись Анны С.: 16:00 → 18:00.
```

Behavior:

- refetch day schedule;
- old slot becomes free;
- new slot becomes occupied;
- avoid duplicate cards;
- if provider was viewing old booking detail, show stale-data notice and updated booking link.
...
provider.schedule.booking_rescheduled_by_ayla.received
provider.schedule.booking_rescheduled_by_ayla.rendered
```
...
31. Calendar reflects Ayla auto-reschedule by moving booking from old slot to new slot.
32. Provider sees post-action notification for Ayla auto-reschedule.
33. Old and new slot states are refetched or updated after reschedule event.
