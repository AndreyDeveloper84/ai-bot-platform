...
|---|---|---|
| смотрящий | `ayla-viewer` | `view_*` на всё, что есть в админке: очередь handoff, диалоги, каталог, журнал. Ни одной кнопки «сохранить». |
| правящий | `ayla-editor` | всё, что видит смотрящий, плюс правку прикладных данных: каталог, брони, задачи handoff, база знаний, персона, промо, расписание. |

Ни одна роль не получает:
...
* правку журналов (`audit`, `admin.logentry`, `events`, `eventbus`,
  `ingress`, `replay`, `consent`) — след, который можно править, не след.
* правку `tenancy.Tenant` — там лежат токен бота и вебхук-секрет.
* `django_celery_beat` — расписание воркеров это эксплуатация, не данные.

Определения — `apps/adminconsole/roles.py`; они же и есть источник
правды, документация лишь пересказывает.

> **Предупреждение про кросс-тенантность выше остаётся в силе.** Роли
> делят *действия*, а не *салоны*. Смотрящий видит данные всех тенантов
> — просто не может их изменить. Разграничение по тенанту — отдельная
> задача (DRF-1022).
