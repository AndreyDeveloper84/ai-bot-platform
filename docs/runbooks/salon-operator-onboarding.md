...
**Экраны админки здесь не описываются.** Что админка умеет как инструмент — какие
экраны есть, что каждый пишет и на чём отказывает — в гайде администратора:
`Ayla/docs/ADMIN_GUIDE_SALONS_AND_MASTERS.md` (сверен с каталогом `b9d19060` и
ботом `8bf99f7c`). Здесь — порядок шагов владельца и readback каждого. Две копии
одной процедуры расходятся, и тогда у оператора два ответа.
...
значения в схеме нет.

Сторож в самой команде готовится (каталог #492: отказ тенанту без мастеров,
подтверждение флагом `--no-masters-ok`). Пока он не слит — служебный тенант от
салона отличает только человек, то есть эта строка. **После слияния команда
откажет сама и назовёт число мастеров; строку тогда снять.**
...
салонный API `/api/v1/tenants/me/…` (нужен шаг 10). **После слияния каталог
#485** формы появятся (`TenantClosureAdmin`, `SpecialistScheduleExceptionAdmin`
в `appointments/admin.py`) — тогда эта строка станет неверной и снимается
вместе с пометкой в гайде администратора.
