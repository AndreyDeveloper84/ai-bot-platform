...
Источник — immutable record **r3** `docs/safety/reviews/OWNER_RULINGS_SAFETY_RECHECK_CONTRACT_2026-09-20_r3.md` (RECORD SHA-256 `4935e344689632cf41ce6622198d70ebbfd87fd4ccb740a8930c5df31e3c3aaf`; дословный ответ владельца «согласен, утверждаем», 20.09.2026; r1 и r2 — `SUPERSEDED BY r3`), реестр [OD-BOT §166] / [§167]. Формулировки строк ниже, которых нет в утверждённом блоке владельца (именованная формула `eligible_basis` / `mandatory_guards`, перечни фраз, детали provenance и state machine Пакета B), — `ENGINEERING ELABORATION OF OWNER RULING`, а не дословное слово владельца; дословный текст — в r3, engineering rendering — `docs/safety/reviews/AYLA_S1_SAFETY_RECHECK_CONTRACT_DELTA_v0.1_2026-09-20.md`. Уточняет [§156] / [§162], не отменяет; семь групп, четыре состояния, owner-блок выше — **не меняются**. Регистрация контракта, не разрешение включить runtime (Пакет B); ни одна строка не является physician verdict.
