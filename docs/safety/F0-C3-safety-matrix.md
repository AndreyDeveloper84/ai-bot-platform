...
| OD-F0C3-09 | Reassessment / resolution policy для S1 `STOP` (V4: не TTL; допустимые источники снятия) | T-S1-07, persistence | Owner. **Owner rulings:** [OD-BOT §156] (снятие только `safety_recheck`), [OD-BOT §161] (recent-resolved), [OD-BOT §162] (восемь условий clearance; outcome `S1_NOT_CURRENT_MEDICAL_FOLLOWUP_REQUIRED`); остаток — точка входа / provenance schema (DRF-2040 q1–q3) |
