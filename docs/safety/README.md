...
| `docs/safety/reviews/OWNER_RULINGS_SAFETY_RECHECK_CONTRACT_2026-09-20.md` | immutable owner record 20.09 — product-контракт `safety_recheck` / `CLEARED_BY_RECHECK` (решения 1–5: точка входа `safety_recheck.start`, формула `ANY(1..5) AND ALL(6..8)`, граница с CLARIFY, provenance contract, Пакет A / B); `DO NOT EDIT — SUPERSEDE WITH A NEW RECORD`; RECORD SHA-256 `e7b54cc3d9a27c6a067d54b7e725122093ca630f59abced86af7ce23b25a6ff1` ([OD-BOT §166–§167]) | — (создан в этом репозитории) |
| `docs/safety/reviews/AYLA_S1_SAFETY_RECHECK_CONTRACT_DELTA_v0.1_2026-09-20.md` | рабочий delta-документ recheck contract v0.1: формула, entry-point / API, state machine (engineering rendering), provenance schema (`IMPLEMENTATION CARRIER: OPEN`), Пакет A / B, fixture delta v0.2.1 → v0.2.2, трассировка к PR #1893 | — |
...
**Версии после owner recheck contract 20.09 (PR docs/safety-recheck-owner-contract):** матрица v0.12-reviewfix3; Review Pack v0.1-reviewfix3 (§7D physician queue); fixtures v0.1.1 (35, без изменений) и v0.2.2 (93 = 69 + 24 REC-contract fixtures, technical `NOT_IMPLEMENTED`, clinical `PENDING_CLINICAL_EXPERT`); SHA изменённых файлов — в шапках файлов (предыдущая версия) и в описании PR. Runtime не менялся; `CLEAR_RESTRICTION ENABLED: NO`; `QUESTION FLOW IMPLEMENTED: PARTIAL` (PR #1893).
