...
- Панель ресепшн — две вкладки: **День · Команда**. «Чаты» (бэкенд отвечает 403), «Настройки» (заглушка) и «Услуги» не показывают.
...
### ✅ Уточнено владельцем 06.09.2026 (DRF-1552) — `docs/OPEN_DECISIONS.md` §35 п.1

Дословно: **«Ресепшн открыть чтение "Дня салона". Только GET, без изменений
записей и настроек. Вкладку "Услуги" у ресепшн убрать, пока для неё нет
разрешённого сценария.»**

Правка уже выложенного (PR #1392). Что изменилось против записи выше:

- Панель ресепшн — **две** вкладки, День · Команда. «Услуги» убраны:
  экран за ними — матрица «мастер × услуга» с правкой и массовым
  применением, а не прайс-лист, и читающего варианта для приёмной роли
  не существует. Прямая ссылка на `/admin/services` даёт тот же честный
  отказ (`AdminSectionDeniedScreen`), что «Чаты» и «Настройки».
- **Права на бэкенде ослаблены ровно на один эндпоинт.**
  `GET /api/v1/admin/day/` перешёл с `require_admin_role` на
  `require_admin_or_reception_read` (`apps/admin_api/auth.py`). Второй
  гейт добавляет ресепшн в список допущенных **только на безопасном
  методе** (GET/HEAD) и проставлен ровно на одной вьюхе.
  `require_admin_role` не тронут: приглашения, деактивация мастеров,
  доступность, ростер, матрица услуг и поиск клиентов по-прежнему
  отвечают ей 403. Запись в §2a «backend прав, `owner|admin` оставить
  как есть» верна для всего, кроме чтения дня.
- Строка выше «`AdminSalonDayScreen` роль не проверяет вовсе» описывала
  фронт; бэкенд за этим экраном ресепшн до 07.09.2026 не пускал, и
  именно это здесь исправлено.
...
2026-09-07 (DRF-1552) — §2a переписан по решению владельца §35 п.1: у ресепшн
две вкладки (День · Команда), `GET /api/v1/admin/day/` открыт ей через
`require_admin_or_reception_read`. Записи §2a и §10 «backend management
owner\|admin оставить» остаются верны для всего остального
`/api/v1/admin/`.
