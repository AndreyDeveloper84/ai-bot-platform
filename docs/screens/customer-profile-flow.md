...
`revoked` (процедура прошла целиком, удаление в Ayla подтверждено чтением каталога),
`revoked_deletion_started` (DRF-1950: удаление в Ayla поставлено в задание и ещё не подтверждено —
экран: «Согласие отозвано.» + «Удаление запущено. Оно завершится в установленный срок.») либо
`revoked_partial_processing` с `failed_steps` —
