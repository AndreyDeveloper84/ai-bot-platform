"""У ``CatalogMaster.address`` читателей НОЛЬ — и это не случайность (§137).
...
2. **Старшинство РЕШЕНО (§137, 11.09.2026) — и не в пользу этого поля.**

   Когда сторож заводился, правила «салон против мастера» (DRF-1589) не
   существовало, и здесь стояло «смысл поля не определён». Это больше не
   так. Оставить прежний текст значило бы держать устаревшее утверждение
   о состоянии решений — и прочтёт его именно тот, кто собрался писать
   читателя, то есть худший из возможных читателей устаревшего.

   Владелец решил дословно: **«Не до мастера, а до места оказания
   услуги»** — салона, кабинета или подтверждённого рабочего адреса
   самостоятельного мастера. Довод: «мастер — человек, у него может не
   быть адреса вовсе, а у места он есть всегда»; этим же снимается
   вопрос приватности — координаты принадлежат месту, а не человеку.

   Смысл ``CatalogMaster.address`` от решения **сузился, а не
   прояснился**: это не адрес мастера-человека, а адрес места ровно в
   одном случае — когда мастер самостоятельный и адрес подтверждён. Во
   всех прочих местом распоряжается ``Tenant.address``.

   **Что это значит для автора читателя.** Читаете поле, чтобы посчитать
   расстояние, — вы, скорее всего, открыли не ту колонку: §137 велит
   считать до места. Читаете как «рабочий адрес соло-мастера» —
   предъявите, где проверяется, что мастер соло и адрес подтверждён: без
   этой проверки поле снова становится адресом человека, а считать до
   человека §137 запрещает.
...
def test_master_address_has_no_readers():
    ...
    Не потому, что читать запрещено, а потому, что §137 велит считать
    расстояние ДО МЕСТА, а это поле — адрес места лишь у соло-мастера с
    подтверждённым адресом. Полный разбор — в докстринге модуля; он
    написан для того, кто увидит это красное.
    """
    offenders = []
    ...
    assert not offenders, (
        ...
        + "салон-против-мастера РЕШЕНО (§137): считать надо ДО МЕСТА, а не до "
        + "мастера-человека. Это поле — адрес места ТОЛЬКО у соло-мастера с "
        + "подтверждённым адресом; иначе местом распоряжается Tenant.address. "
        + "Подстановка пустой строки вместо None вдобавок выдаёт наше "
        + "незнание за ответ источника.\n"
