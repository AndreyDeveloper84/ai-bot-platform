...
def _auth_step(workflow_name: str) -> dict:
    ...
    # Раньше здесь стояло «ровно один», и довод был: «если шаг раздвоился —
    # раздвоится и поведение». Довод верный, а мера — нет: она запрещала не
    # расхождение, а КОЛИЧЕСТВО. Разрезание набора на шарды (11.09.2026) дало
    # второе задание, которому тоже нужна авторизация перед `uv sync`, — и
    # страж покраснел на изменении, к его предмету отношения не имеющем.
    #
    # Теперь проверяется ровно то, что защищалось: шагов может быть сколько
    # угодно, но все они обязаны быть одним и тем же шагом. Два одинаковых
    # разойтись не могут; два разных — вот беда, и её страж ловит по-прежнему.
    assert found, (
        f"{workflow_name}: не найдено ни одного шага с GH_DEPLOY_TOKEN и insteadOf. "
        f"Либо авторизацию убрали, либо страж смотрит не туда — оба случая требуют разбора."
    )
    first = found[0]
    for other in found[1:]:
        assert other == first, (
            f"{workflow_name}: шаги авторизации РАЗОШЛИСЬ. Найдено {len(found)} штук, "
            f"и они не одинаковы. Расхождение означает, что одно задание настраивает "
            f"git иначе, чем другое, — это и есть раздвоенное поведение."
        )
    return first
