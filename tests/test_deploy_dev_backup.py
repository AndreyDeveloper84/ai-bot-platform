...
def test_the_step_says_who_writes_and_where(backup: dict) -> None:
    ...
    said = next((i for i, ln in enumerate(lines) if "id -un" in ln), -1)
    dump = next((i for i, ln in enumerate(lines) if "pg_dump" in ln), -1)

    assert said != -1, "шаг не называет пользователя, под которым пишет"
    assert dump != -1, "шаг перестал делать дамп — проверять нечего"
    assert said < dump, f"имя пользователя печатается ({said}) ПОСЛЕ дампа ({dump})"
