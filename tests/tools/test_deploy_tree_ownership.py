...
def test_the_recorded_baseline_is_the_measured_one() -> None:
    ...
    * **3410 → 298.** Дерево починили, и 298 — это продукция **одного**
      прогона выкладки по чистой базе. То есть теперь это не накопленный долг,
      а **скорость утечки**, и ровный счёт больше не означает «ничего не
      происходит».
    """
    baseline = read_baseline()

    assert baseline[GIT] == 67
    assert baseline[STATICFILES] == 154
    assert baseline[OTHER] == 77
    assert sum(baseline.values()) == 298
