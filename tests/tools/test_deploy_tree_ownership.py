...
def test_foreign_objects_land_in_the_right_buckets() -> None:
    ...
    assert found == {GIT: 2, STATICFILES: 1, PYCACHE: 0, OTHER: 1}
...
def test_the_recorded_baseline_is_the_measured_one() -> None:
    ...
    существующие объекты свежей регрессией, свалив вину на того, кто оказался
    рядом.

    Число менялось дважды, и оба раза поучительно:

    * **3315 → 3410.** Первый замер снят на **другой машине** — заброшенной
      копии контура по тому же пути. Путь спора не решил, потому что он есть на
      обеих; решило то, что предмет назвал себя сам.
    * **3410 → 298.** Дерево починили, и 298 — это продукция **одного**
      прогона выкладки по чистой базе. То есть это была не накопленный долг,
      а **скорость утечки**.
    * **298 → 561 → осадок.** Источник закрыт (#1586, #1588): первая полная
      выкладка под владельцем дала **0 свежих чужих**. 561 — то, что root
      оставил до починки и что никто с тех пор не переписывал. Оно не растёт
      само и не убывает само; храповик теперь стережёт **рост**, а рост
      означает, что течь открылась снова.
    """
    baseline = read_baseline()

    assert baseline[GIT] == 303
    assert baseline[STATICFILES] == 153
    assert baseline[OTHER] == 105
    assert sum(baseline.values()) == 561
...
def test_growth_in_one_bucket_fails_even_when_the_total_shrinks() -> None:
    ...
    """
    baseline = {GIT: 2130, STATICFILES: 174, PYCACHE: 0, OTHER: 1011}
    found = {GIT: 2200, STATICFILES: 0, PYCACHE: 0, OTHER: 1011}
...
def test_shrinking_is_reported_but_does_not_fail() -> None:
    baseline = {GIT: 2130, STATICFILES: 174, PYCACHE: 0, OTHER: 1011}
    found = {GIT: 2130, STATICFILES: 0, PYCACHE: 0, OTHER: 1011}
