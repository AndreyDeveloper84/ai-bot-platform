...
**Поверхность за границей `apps/`.** `legacy_maxbot/` не сканируется, и
причина не в том, что его код безопасен — он НЕ безопасен:
`ai_ui.py::render_water_prompt` рисует «💧 Сегодня: 1.2 л / 0 мл»
безусловно, без всякой проверки нормы на положительность.

Причина в том, что этот код НЕ ИСПОЛНЯЕТСЯ. Проверено, а не предположено:
ни одного `import legacy_maxbot` вне самого каталога; сам импорт объявлен
banned-api в `pyproject.toml`; каталог исключён из ruff, mypy и
`norecursedirs` pytest; `legacy_maxbot/MIGRATION_NOTICE.md` называет его
замороженным read-only снимком `mysite/maxbot/` от 2026-05-09, который
сливается в `apps/` по спринтам и удаляется целиком.

Если он когда-нибудь оживёт — сюда надо добавить путь, а те места
починить.
